...
## ledger account
A node in the chart of accounts. **Eight v1 classes**: `firm_treasury`, `psp_clearing`, `fees_revenue`, `reserve`, **`trader_withdrawable`** (per identity), **`trader_wallet`** (per identity), `promotional_credit`, `withdrawals_in_flight`. The account-type enum is expandable by design.
...
**`withdrawals_in_flight` is the eighth class and the only firm-scoped `liability` in the chart** ([ADR-187](decisions/ADR-187.md), [`0056`](../packages/db/migrations/0056_eighth_ledger_code.sql)). It carries the external leg's in-flight obligation: `LT-06` credits it when a wallet withdrawal is approved and the trader's wallet claim is extinguished, and `LT-07` debits it when the cash leaves, so it stands at a credit balance for exactly the interval between approval and settlement. **This list is the control that caught the last invented class**: [`0027`](../packages/db/migrations/0027_triggers_invariants.sql) records that the first draft of [ADR-027](decisions/ADR-027.md) invented `firm_payable` and that it was caught by reading this entry. A migration that widens the vocabulary while this line still says seven has disabled that control, which is why the count moves here in the same wave as the schema.
