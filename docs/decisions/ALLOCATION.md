...
| **044** | `claude/builder-reviewer-loop-rykvhs`, this session | **reserved, unwritten.** The `trading_calendar` prior image becomes a trigger. [ADR-042](ADR-042.md) was silent and the founder has ruled it |
...
| **0033** | `claude/builder-reviewer-loop-rykvhs`, `ADR-044` | **reserved, unwritten.** `0033_trading_calendar_revision_required.sql`, the trigger that makes [ADR-042](ADR-042.md) F-2's prior image a control rather than a rule the loader follows. **This row is committed and pushed BEFORE the migration file exists**, on the founder's instruction and on the reading below |
