...
This ADR named four things stale in [DATA_MODEL section 11](../architecture/data-model/README.md)'s example: `ladder` 8, `win_days.required_count` 5, `phase_eval.min_trading_days` 1, and the singular `kyc.placement`. **Only the first and fourth were stale.** Those two are the key-name rulings and they are folded.
