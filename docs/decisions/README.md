...
| [ADR-096](ADR-096.md) | an unauthenticated public read goes over HTTP to the public API, and `apps/site` never links `packages/db` at all  (2026-08-24, status: proposed) |
| [ADR-097](ADR-097.md) | OUTCOME 1 OF THE THREE. The vocabulary ALREADY HAS the term, it is `outside-loader-boundary`, and `GS-143` and `GS-144` are MIS-FILED. No seventh term, no row leaves section 39, and the closed six stay closed  (2026-08-24, status: proposed) |
<!--/gen-->
