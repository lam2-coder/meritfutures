...
**One engineering question is open underneath the telephony row and counsel should know it exists, because its answer changes what this policy can describe.** Merit's contact addresses are stored as one-way hashes rather than as addresses, which is a minimization posture. **Whether that survives contact is [M10](../plans/M10-integrations.md) `OQ-M10-06`**: a security notice to a prior address, which is an account-takeover countermeasure Merit intends to run, needs an address it can send to. If the resolution is to hold addresses **reversibly encrypted** rather than not at all, the retention description for this category changes, and it is better for counsel to see the question than to be given a description that moves after the sitting.
