...
| LT-09 | `wallet_withdrawal_failure` | debit **`withdrawals_in_flight`** `amount_cents`; credit **`trader_wallet`** (identity) `amount_cents`, with `reversal_of` set to `LT-06`'s transaction (`SD-M5-05`). **THE EXACT NEGATION OF `LT-06`, AND IT IS A COMPENSATING ENTRY AND NEVER AN UPDATE** ([ADR-189](../decisions/ADR-189.md)): `LT-06` is not deleted, not amended, and no balance is adjusted in place. **What triggers it is `transferring --> failed`, [STATE_MACHINES section 3.2](../architecture/STATE_MACHINES.md)'s `G-TRANSFER-EXHAUSTED`, and that is the ONLY terminal state that strands anything**, checked state by state rather than assumed: `settled` discharges the obligation through `LT-07`, and `cancelled` is reachable only from `requested` and `cooling`, both BEFORE approval, so `LT-06` never posted and there is nothing to reverse. **It posts under `reference_kind` `wallet_withdrawal` against the withdrawal's id**, the same tuple `LT-06` and `LT-07` use, because `ledger_entries` carries no withdrawal foreign key and that pair is the whole of the edge from a withdrawal to its postings. Its `ledger_transactions.idempotency_key` is `` `wallet_withdrawal_failure <the withdrawal's stored key>` ``, which is [ADR-175](../decisions/ADR-175.md) clause 3's spelling extended to a third posting rather than assumed to cover it. **THE REVERSAL IS PROVED RATHER THAN PROMISED, AND THAT IS WHAT [`0057`](../../packages/db/migrations/0057_terminal_withdrawal_obligation.sql) IS**: `WD-C1` asserts at every terminal transition that the net of `withdrawals_in_flight` over every transaction naming the withdrawal is exactly **0**, and `ledger_transactions_reversal_of_idx` becomes UNIQUE so that one transaction is reversed at most once. **Executed end to end against PostgreSQL 16** on the per-identity rows [`0054`](../../packages/db/migrations/0054_identity_ledger_accounts.sql)'s trigger opens, after an `LT-01` of 30,000 cents split 25,000 trader and 5,000 firm: `trader_wallet` **-25000** after `LT-01`, **0** after `LT-06` with the obligation at **-25000**, the transition to `failed` REFUSED while the obligation stood, and after `LT-09` the obligation **0** and the trader's wallet claim back at **-25000**, global sum **0**. The external leg's failure |
...
**THE TWO OPEN SLOTS ARE FILLED AND THE ROWS ARE POSTABLE** ([ADR-187](../decisions/ADR-187.md)). The eighth ledger code, `withdrawals_in_flight`, is minted by [`0056`](../../packages/db/migrations/0056_eighth_ledger_code.sql) and seeded there, and it is the only firm-scoped `liability` in the chart. **The amendment is ADDITIVE on [ADR-180](../decisions/ADR-180.md)'s and [ADR-181](../decisions/ADR-181.md)'s precedent**: both rows keep every word they carried, including the quotation of what they used to read and the `NOT RULED` history, and gain only the code. **LT-02's counterparty slot is NOT filled and this entry does not fill it.** That row says what replaces the pooled class is *"[ADR-174](../decisions/ADR-174.md) section 3's question"*, which a reader could take to mean [ADR-187](../decisions/ADR-187.md) answers it; it does not. [ADR-174](../decisions/ADR-174.md) clause 4 derives this role from a gap BETWEEN TWO MOMENTS that exists only on the external leg, and says in terms that the internal leg *"had no such gap because [M05](M05-payout-system.md) section 3.1 makes the internal leg one transaction"*. **LT-02 stays UNPOSTABLE and stays a separate ruling.**

**THE EXTERNAL LEG HAD A TERMINAL STATE WITH NO POSTING, AND `LT-09` IS IT** ([ADR-189](../decisions/ADR-189.md)). [STATE_MACHINES section 3.2](../architecture/STATE_MACHINES.md) draws `transferring --> failed: G-TRANSFER-EXHAUSTED` and rows it with `wallet.withdrawal_failed`, and the table above declared eight transactions of which not one was a wallet withdrawal that failed on the rail. **The gap was not hypothetical once [ADR-187](../decisions/ADR-187.md) minted the account it strands.** `LT-06` posted, so the trader's wallet claim is extinguished; `LT-07` never posts, so no cash leaves; `withdrawals_in_flight` carries a credit balance against a withdrawal that will never settle, and **the trader's money is in neither place they could be shown it.** **The mechanism was already ruled and is not redesigned here**: `SD-M5-05` carries `reversal_of` and corrections are compensating entries and never updates, so the ninth transaction REVERSES `LT-06` rather than deleting it, amending it, or adjusting a balance in place. **`LT-06`'s and `LT-07`'s rows are NOT amended by this entry and neither is `LT-02`'s.** The ninth transaction is a new row and it takes nothing from the three that were open; what it needed from them, the account and its direction, they already say.
