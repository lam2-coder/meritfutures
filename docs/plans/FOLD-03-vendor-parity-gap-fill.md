...
| # | Question |
|---|---|
| **OQ-F3-01** | **The eligibility trade in `ADR-067`.** Excluding adjustments from eligibility makes a goodwill credit unwithdrawable; admitting them lets an adjustment manufacture an obligation under zero denial. **Which, and is a flagged admission acceptable?** **RULED by [ADR-067](../decisions/ADR-067.md) section 3, 2026-08-20, approved 2026-08-21**: neither horn, because the pair is not exhaustive. Excluded from the payout computation and withdrawable on the wallet rail. **The flagged-admission half does not arise**: the admission horn describes a door that does not exist. **Amended in place by [ADR-135](../decisions/ADR-135.md) clause 1**, which also records the residue: [ADR-075](../decisions/ADR-075.md) shut the wallet exit for a `closed` identity on 2026-08-21, so the ruling holds for `active` and `restricted` and strands the credit on `closed` |
| **OQ-F3-02** | **Is a debit permitted at all?** `INV-M6-03` forbids an admin action that denies a payout. A debit against a balance is not that, and it is adjacent enough to need saying **RULED by [ADR-067](../decisions/ADR-067.md) section 4, 2026-08-20, approved 2026-08-21**: permitted in exactly one shape, the exact reversal of a credit this table itself posted, enforced in [`0038`](../../packages/db/migrations/0038_account_adjustments.sql) by a `CHECK`, a partial unique index and the `ADJ-C1` trigger. **CLOSED WITH NO RESIDUE** ([ADR-135](../decisions/ADR-135.md) clause 6): the permission is a property of the credit being reversed and of nothing about the person, so no identity-status amendment can reach it |
