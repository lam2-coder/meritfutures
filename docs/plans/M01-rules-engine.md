...
**R-04 stays `Engine` and R-06 stays `Engine`, deliberately.** R-04 is implemented and exercised on both branches; what is missing is a publisher for the `halted` input, which is a retrieval question and is recorded as one in [section 10](#10-open-questions-and-what-the-founder-ruled), not a layer boundary. R-06 is the engine's own forward-only fold and needs a clock only to write the scenario that pins it, never to execute the rule.
