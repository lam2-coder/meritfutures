...
| INV-06 | The floor never decreases, **except at the R-31 funded reset**, where it is initialised to `size_cents` minus the **funded** drawdown, equivalently `size_cents - funded drawdown_cents` (R-12, R-31, GS-019). No other exception, no phase qualifier, no settlement carve-out ([ADR-014](../decisions/ADR-014.md), amended by [ADR-050](../decisions/ADR-050.md)) | Property RE-P-01, GS-010, GS-019, GS-081 |
...
interface ExternalGates {              // context, never replayed (INV-23)
  ...
  hasPayoutInFlight: boolean;          // approved | frozen | held_pending_review for this account (ADR-040, ADR-254)
}
...
| SD-09 | `payout_requests` | add unique partial index on `(account_id) where status in ('approved','frozen','held_pending_review')` | Enforces G-NO-IN-FLIGHT in the database, because the engine is not the only writer (EC-040). **THIS ROW READ `('approved','transferring','frozen')` until [ADR-254](../decisions/ADR-254.md)**, which is the value [ADR-028](../decisions/ADR-028.md) retired from the table and the one [ADR-040](../decisions/ADR-040.md) added in its place. The index itself has carried the live set since [`0031`](../../packages/db/migrations/0031_payout_hold_and_identity_restriction.sql); this cell is the last of the four sites ADR-040's own sweep named and did not reach |
...
| R-38 | One payout in flight | **Engine** | n/a | **ACCOUNT GRAINED** ([ADR-254](../decisions/ADR-254.md)): no outstanding payout request **for this account** in `approved`, `frozen`, or `held_pending_review`. That is `payout_requests_no_in_flight_uq`'s predicate written out ([`0031`](../../packages/db/migrations/0031_payout_hold_and_identity_restriction.sql)), because a gate that disagrees with its own index is a gate that holds on Tuesdays. [ADR-019](../decisions/ADR-019.md) moved G-NO-IN-FLIGHT to the external leg on the premise that the internal leg had no in-flight window to stack inside; [ADR-040](../decisions/ADR-040.md) put `held_pending_review` BEFORE approval, which restored that window, and re-created this index at the account rather than dropping it. **ADR-019's one-in-flight-per-identity rule survives on the external leg and is a different rule from this one**: its object is `wallet_withdrawals`, and because that leg's open index is plain rather than unique it is enforced by the withdrawal handler ([ADR-158](../decisions/ADR-158.md) finding 8) and never by this input. **THIS ROW READ** *"Applies to the external leg only (ADR-019): no wallet-to-rail withdrawal for this identity in `approved`, `transferring`, or `frozen`. The internal leg completes in one transaction, so there is no window for a second request to arrive inside and AS-01 is structurally resolved rather than gated"* **until [ADR-254](../decisions/ADR-254.md)**. It is quoted rather than deleted, because a sentence removed leaves nothing for the next reader to check, and because it is the reading that reached `types.ts`. Its status set was satisfiable on neither table: `payout_status` has no `transferring` and `wallet_withdrawal_status` has no `frozen`. | GS-052, GS-053 |
...
| R-38 | One payout in flight | n/a | **ACCOUNT GRAINED** ([ADR-254](../decisions/ADR-254.md)): no outstanding payout request **for this account** in `approved`, `frozen`, or `held_pending_review`. That is `payout_requests_no_in_flight_uq`'s predicate written out ([`0031`](../../packages/db/migrations/0031_payout_hold_and_identity_restriction.sql)), because a gate that disagrees with its own index is a gate that holds on Tuesdays. [ADR-019](../decisions/ADR-019.md) moved G-NO-IN-FLIGHT to the external leg on the premise that the internal leg had no in-flight window to stack inside; [ADR-040](../decisions/ADR-040.md) put `held_pending_review` BEFORE approval, which restored that window, and re-created this index at the account rather than dropping it. **ADR-019's one-in-flight-per-identity rule survives on the external leg and is a different rule from this one**: its object is `wallet_withdrawals`, and because that leg's open index is plain rather than unique it is enforced by the withdrawal handler ([ADR-158](../decisions/ADR-158.md) finding 8) and never by this input. **This is a context gate and not a term in `engineEligible`**: its input `hasPayoutInFlight` is declared on `ExternalGates` (section 2.1), it never enters the replayed state or its hash (INV-23, SD-06), and it binds through `contextEligible` at read time, so it is part of `eligible` by R-41 (INV-15, [ADR-060](../decisions/ADR-060.md)). **THIS ROW READ** *"Applies to the external leg only (ADR-019): no wallet-to-rail withdrawal for this identity in `approved`, `transferring`, or `frozen`. The internal leg completes in one transaction, so there is no window for a second request to arrive inside and AS-01 is structurally resolved rather than gated"* **until [ADR-254](../decisions/ADR-254.md)**. It is quoted rather than deleted, because a sentence removed leaves nothing for the next reader to check, and because it is the reading that reached `types.ts`. Its status set was satisfiable on neither table: `payout_status` has no `transferring` and `wallet_withdrawal_status` has no `frozen`. | GS-052, GS-053 |
...
| `GET /admin/eligible-forecast` | `evaluatePayout` projected forward over the calendar | Projects which accounts clear their gates in the next 7 trading days, aggregated at identity level as well as account level (AS-09). **AMENDED BY [ADR-204](../decisions/ADR-204.md), AND THIS ONE CORRECTS THE MIDDLE CELL RATHER THAN EXTENDING IT.** `evaluatePayout` takes a `RuleState`; a `RuleState` for a day that has not closed comes only from `advanceDay`, whose `DayInput.mark` is a `DailyMark` R-06 guarantees does not exist. **So the producer named here has no argument and cannot be called**, which is why four sessions measuring this field found a named producer and no derivation. **Of the eleven conditions the evaluation conjoins, exactly one -- R-37, the cadence gap -- has an input a stored row already fixes for the whole horizon**; the other ten clear only when the trader TRADES, when a balance moves, or when an operator acts. What the endpoint may project is therefore R-37 under five assumptions that entry states and a producer may not choose, over the accounts eligible at the last closed day plus those whose only remaining wait is the cadence gap. **The last cell stands as the statement of what the panel is FOR**, and section 3c of that entry records that its identity-level half is served by no field and no column today |
...
| ID | Property |
|---|---|
| RE-P-01 | `floor(d+1) >= floor(d)` for **every** generated day sequence, for both drawdown types, **including sequences containing settlements**. Since [ADR-014](../decisions/ADR-014.md) the "no settlement" qualifier is gone, which strengthens the property and shrinks the generator's exclusion list to nothing. **The one exception is the day that emits `phase.passed`** ([ADR-050](../decisions/ADR-050.md)): there the floor is asserted to equal `size_cents - funded drawdown_cents` **exactly**, so the R-31 reset is pinned rather than excused. The generator is told nothing about phases; the exception is recognised from the engine's own event stream |
