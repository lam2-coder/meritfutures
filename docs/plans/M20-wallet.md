...
[ADR-019](../decisions/ADR-019.md), Appendix B5's ten-section template, Appendix D4's payout-path hardening, and [SECURITY section 4.7](../architecture/SECURITY.md)'s wallet account-takeover blast-radius analysis. **Money path under [ADR-003](../decisions/ADR-003.md)'s strict regime**, and the newest crown jewel in the estate.
...
| INV-M20-16 | **The impersonation refusal is an authorization decision about the SESSION and is never a member of INV-M20-06's set** | [ADR-068](../decisions/ADR-068.md) requirement 1, section 3.7. INV-M20-06 enumerates **context gates on the identity**: `payouts_frozen`, `recon_blocked`, KYC state and `identities.status`. Each is a fact about the trader and answers the same way whoever is asking. **A session-type term inside that set would make a per-identity gate set answer differently depending on who is looking**, and the first clause the whole invariant rests on, *"every context gate that blocks the external leg blocks wallet spend too"*, would stop being a statement about the identity at all. The two refusals are evaluated at different moments against different subjects and they are kept apart on purpose |
...
### 3.7 What an impersonation session meets on the two exits (ADR-068)

**M20 carries one of [ADR-068](../decisions/ADR-068.md)'s four explicit refusals and one of its three inherited ones.** [ADR-068](../decisions/ADR-068.md) is the authority, [`0042`](../../packages/db/migrations/0042_impersonation_sessions.sql) is the session-type boundary, and `M6-N-02` and `M6-N-03` are claimed in [M06](M06-admin-ops-console.md) section 8.1a.

| Route | Refused by | What this module adds |
|---|---|---|
| `POST /checkout` with `payment_method = wallet` | **explicit** (`M6-N-02`) | INV-M20-16, and the payment-method rule below |
| `POST /wallet/withdrawals` | **`C-27`, inherited** (`M6-N-03`) | **Nothing.** [M05](M05-payout-system.md) section 3.6 owns the route and says why |

**INV-M20-16 is the substantive addition and it is a refusal to widen an invariant.** The obvious way to write "an impersonation session cannot spend the wallet" is to add a term to INV-M20-06, which is already the enumerated list of everything that blocks this door. **It is the wrong place and the reason is the invariant's own first clause.** INV-M20-06 says *"every context gate that blocks the external leg blocks wallet spend too"*, and every member of that set is a **fact about the identity**: `payouts_frozen`, `recon_blocked`, the KYC state, `identities.status`. Those answer the same way whoever asks. **Impersonation is a fact about the session**, and a set that mixed the two would answer differently depending on who is looking, which is not a context gate and would quietly make section 3.2's diagram and section 3.3's request-time check untrue as written. **The two refusals fire at different moments against different subjects**: the impersonation refusal at authorization, against the session; INV-M20-06 inside the spend transaction, against the identity.

**`GS-302` is where that distinction earns its keep, and it also exposes something about the scenario worth recording.** `GS-302` impersonates a **`restricted`** identity, is permitted for visibility, and requires that *"every money surface `restricted` already blocks stays blocked"*. **Both controls refuse a wallet spend in that scenario, so the assertion cannot discriminate which one did it.** That is acceptable as an **outcome** pin and it is not evidence that INV-M20-06 still fires: if the restriction leg were dropped tomorrow, `GS-302` would stay green. **The discriminating assertion is INV-M20-06 exercised from a trader session on a restricted identity**, which is the restriction lifecycle's job and not this scenario's. No ordinal is claimed for it here, on section 8.3's rule and because [M06](M06-admin-ops-console.md) is not this session's to write in.

**The refusal is placed before the payment-method branch, and this module has already paid for learning that once.** [M03](M03-billing-checkout.md) INV-M3-15 refuses a restricted identity's purchase **at the resolved-identity step where every other hard limit sits**, and the finding that put it there is that `mixed` was refusing on the wallet leg **after the card leg was underway**, reaching the right answer through a compensation path. The rule that came out of it is that **every hard limit is asserted against all three payment methods**, because a case asserted against one passes while another refuses for an unrelated reason. `M6-N-02` is written against `payment_method = wallet` and `M6-N-06` against `POST /checkout`; **neither names `mixed`**, and a `M6-N-06` run with `psp` would say nothing about it. **One authorization decision before the branch refuses all three**, and the assertion should be run against all three.

**`M6-N-06`, the purchase refusal, is [M03](M03-billing-checkout.md)'s surface and is not in this session's fence.** It is recorded here rather than left to be discovered: the card and `mixed` legs of `POST /checkout` need the same refusal at the same point, and this document can name the requirement and cannot land it.

**The external withdrawal gets no new enforcement, and [M05](M05-payout-system.md) section 3.6 carries the argument once.** In short: `C-27` already refuses it from any session that cannot elevate, and [`0042`](../../packages/db/migrations/0042_impersonation_sessions.sql) carries no `elevated_at` and no `elevated_by_factor`, so there is no column an elevation could be written to. **It is repeated here in one sentence and argued there in one place**, which is the same discipline INV-M20-16 is protecting.
...
| `wallet.reconciliation_failed` **NEW** | INV-M20-10's nightly per-identity assertion fails | `{ identity_id, expected, actual }`. **Pages**, and it is an [ADR-016](../decisions/ADR-016.md) scoped-halt input |
...
| FM-M20-10 | Per-identity position diverges from the ledger | A wallet-shaped hole, invisible in a global sum that still balances | Nightly per-identity assertion (INV-M20-10) | `wallet.reconciliation_failed` pages and scopes a halt per [ADR-016](../decisions/ADR-016.md) |
...
**Attack.** [ADR-019](../decisions/ADR-019.md) and [M14](M14-loyalty-retention.md) INV-M14-10 separate `promotional_credit` from `trader_wallet` with no path between them, and [M17](M17-offers-engine.md) INV-M17-08 asserts it again. That closes the **direct** route and leaves the product open as an indirect one:
...
**Attack.** Nobody proposes operating a payment institution. What gets proposed, in order, each reasonable: let traders **top up** the wallet so they can buy faster; let a trader's **employer or backer** fund it; let a wallet balance be **sent to another trader** as a gift or a split; pay a small **return** on idle balances to encourage traders to leave money in. [ADR-019](../decisions/ADR-019.md) already rejected interest and peer-to-peer transfer "outright, see the legal note", and it did not reject top-ups, because nobody had proposed them yet.
...
3. **No interest and no peer-to-peer**, per [ADR-019](../decisions/ADR-019.md), unchanged, and now with the reason recorded in the same place as the rest of the boundary.
4. **A counsel item is filed in [legal/](../legal/README.md)** confirming that a payable of this shape is not a regulated stored-value product in Merit's operating jurisdictions, before launch rather than after. [ADR-019](../decisions/ADR-019.md) already flagged a wallet counsel-review item; this scenario states precisely which question it must answer. EC-135, GS-225.
...
3. **Gifting is not a feature and will be requested.** The answer is recorded in advance: a gift is a transfer, a transfer is prohibited by INV-M5-14, and the reason is legal as well as anti-abuse ([ADR-019](../decisions/ADR-019.md)'s rejection of peer-to-peer, AS-M20-04).
...
| GS-231 | Per-identity reconciliation with a global ledger that still balances | The per-identity assertion **fails and pages** even though the global sum is zero. INV-M20-10, [ADR-016](../decisions/ADR-016.md) |
...
| Withdrawal rate: share of credited value withdrawn within 7, 30, and 90 days | The behavioural assumption [ADR-019](../decisions/ADR-019.md)'s liquidity claim rests on, measured rather than assumed |
...
| Per-identity reconciliation failure | any | **page**, and it scopes a halt per [ADR-016](../decisions/ADR-016.md) |
...
**OQ-M20-03 (as asked). Counsel confirmation that the wallet is a payable and not a regulated stored-value product.** [ADR-019](../decisions/ADR-019.md) flagged a wallet counsel item and AS-M20-04 states the precise question: does a balance that receives only earned funds and refunds of its own spend, pays no interest, permits no transfer, accepts no deposit, and is payable on demand, avoid money-transmission or e-money characterization in Merit's operating jurisdictions? **This should be answered before launch**, because the answer may add a condition rather than a prohibition, and conditions are cheap in advance.
...
**OQ-M20-07. A `closed` identity cannot request a payout and can still spend its wallet. Which shape do the other four doors take?**

**The fact, cited rather than described.** [`0001_extensions_and_enums.sql:27`](../../packages/db/migrations/0001_extensions_and_enums.sql) declares `identity_status AS ENUM ('active', 'restricted', 'closed')` and [ADR-041](../decisions/ADR-041.md) refused a fourth value. [ADR-062](../decisions/ADR-062.md) section 1 rules the payout gate `identities.status = 'active'`. INV-M20-06 at line 62 of this document enumerates the wallet-spend and external-withdrawal set with `identities.status = 'restricted'`, the `<>` shape. **`closed` fails the first and passes the second**, so after [ADR-062](../decisions/ADR-062.md) a `closed` identity cannot request a payout and can still spend its wallet balance, and the external-withdrawal door behind it is open on the same predicate.

**The door inventory, which is the part nobody had assembled.** Five doors read `identities.status`. **One is written one way and four are written the other:**

| Door | Site | Shape today |
|---|---|---|
| Payout request | `G-ELIGIBLE`, [STATE_MACHINES](../architecture/STATE_MACHINES.md) section 10 | **`= 'active'`**, ruled by [ADR-062](../decisions/ADR-062.md) |
| External withdrawal, at approval | `G-WITHDRAWAL-CLEARED`, same table | the identity **not `restricted`** |
| Wallet spend and withdrawal, at request | **INV-M20-06, this document** | `= 'restricted'` blocks |
| Purchase and reset, all three payment methods | [M03](M03-billing-checkout.md) INV-M3-15 | `restricted` blocks |
| Affiliate statement settlement | [M08](M08-affiliate-system.md) INV-M8-12 | `restricted` holds |

**Proposed: `identities.status = 'active'` on every door.** That is [ADR-062](../decisions/ADR-062.md)'s own recommended remedy, *"to move the other doors to `= 'active'` rather than to move the payout gate back"*, and its argument is that `<>` **fails open** on every value the enum gains later while `=` fails closed. **One shape written two ways is the defect this corpus names more often than any other**, and today it is written two ways across five doors on the money path.

**Why this session proposes it and does not land it, and the reason is checkable rather than a fence excuse.** `G-WITHDRAWAL-CLEARED`'s primary source **is** INV-M20-06. [Session 78](../sessions/2026-08-20-session-78.md) held [STATE_MACHINES](../architecture/STATE_MACHINES.md) and deliberately did not move that guard, on the ground that changing it without M20 would make the two documents disagree, *"which is strictly worse than the divergence"*. **Moving INV-M20-06 without [STATE_MACHINES](../architecture/STATE_MACHINES.md) creates the identical disagreement from the other side.** The two files move together or neither moves, and [STATE_MACHINES](../architecture/STATE_MACHINES.md) is not in this session's fence. **And [ADR-062](../decisions/ADR-062.md) is `status: proposed` with an unsigned approval line**, so adopting its predicate here would propagate an unsigned money-path ruling into four further documents on a session's authority.

**What is different after this entry is the ownership, and the ownership was the defect.** [ADR-062](../decisions/ADR-062.md) numbered it `OQ-062-01`; [ADR-068](../decisions/ADR-068.md) section 7 restated it and recorded that **no session's fence contained the file**; [FOLD-04](FOLD-04-impersonation-and-admin-parity.md) carried it forward again. **It now has a number in the namespace of the document that contains its site**, an inventory of every door it touches, and a predicate to accept or refuse. `OQ-062-01` and `OQ-F4-04` are not renumbered and are not duplicates of this: one is the ruling's own record of the exposure and the other asks whether a `closed` identity may be impersonated at all, which [`0042`](../../packages/db/migrations/0042_impersonation_sessions.sql) leaves deliberately unconstrained.

**Two things the founder should decide separately.** The four doors are not one kind of door. **Payout, external withdrawal and wallet spend are extraction**; purchase and affiliate settlement are not, and a `closed` identity that can still buy is a different question from one that can still take money out. **A single answer for all five is available and is not obviously the right one.** The other is scope: this is a predicate, not a mechanism, and the change is three or four cells of text plus the tests that pin them, which is why it is cheap today and is [ADR-062](../decisions/ADR-062.md) section 1's own third argument for ruling it at all.
