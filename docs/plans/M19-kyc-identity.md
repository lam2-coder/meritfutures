...
| INV-M19-15 | **No impersonation session creates or re-creates a verification**, and the refusal is refused twice over | [ADR-068](../decisions/ADR-068.md) requirement 1 and `M6-N-07`, section 3.5. It is an authorization refusal, **and it is the only one on [ADR-068](../decisions/ADR-068.md)'s list whose harm is not an authorization harm**. A verification row is **attributed to its subject**, records the `placement` that produced it, and enters the funnel INV-M19-11 captures at the time and says cannot be reconstructed afterwards. So an operator-initiated session the subject never completes writes an abandonment against a real human and into the dataset section 10's decision is to be settled by. **A refusal that would still be right if the impersonation machinery vanished is a refusal worth stating in this module rather than only in the ADR** |
...
### 3.5 What an impersonation session meets on this module's routes (ADR-068)

**M19 carries one of [ADR-068](../decisions/ADR-068.md)'s four explicit refusals and the ceremony behind one of its three inherited ones.** [ADR-068](../decisions/ADR-068.md) is the authority, [`0042`](../../packages/db/migrations/0042_impersonation_sessions.sql) is the session-type boundary, and `M6-N-05` and `M6-N-07` are claimed in [M06](M06-admin-ops-console.md) section 8.1a.

| Route | Refused by | What this module adds |
|---|---|---|
| `POST /kyc/session` and `POST /kyc/reverify` | **explicit** (`M6-N-07`) | The second reason, below, and INV-M19-15 |
| `POST /phone/change` and its cancel | **`C-27`, inherited** (`M6-N-05`) | **Nothing.** The ceremony is this module's and the refusal is not |

**The KYC refusal is the one on [ADR-068](../decisions/ADR-068.md)'s list that would still be right if impersonation did not exist**, and that is worth writing down where the route lives. [FOLD-04](FOLD-04-impersonation-and-admin-parity.md) section 5.2 states the reason in one line: **a KYC submission is an identity assertion whose whole value is that the subject made it.** INV-M19-06 says a re-verification is a **new** verification and never a re-read of a stored result, and AS-M19-06 is the scenario where that property is satisfied on paper and verifies nothing. **An assertion presented by somebody who is not the subject is the same failure with a nicer name on it.**

**Two harms are certain and neither of them is the authorization harm.**

1. **The row is attributed to the trader.** A verification is a claim about a named human, and one created inside an impersonation session says that human presented themselves when they did not. This is [ADR-069](../decisions/ADR-069.md)'s provenance argument arriving at the one table where provenance **is** the product: INV-M19-12 already requires that enforcement resting on this evidence be defensible **independently of the provider relationship**, and evidence whose subject did not make it is not defensible anywhere.
2. **The telemetry is corrupted at the point of capture.** `POST /kyc/session` writes a row carrying its `placement` and enters the funnel, and INV-M19-11 records placement, cost and funnel position **at the time it happens** precisely because *"data not captured at the time cannot be reconstructed"*. An operator-initiated session the subject never completes is an abandonment against a real identity, in the dataset section 10's placement decision is to be settled by. **A refusal that also protects a measurement is cheap to keep and expensive to notice missing.**

**`OQ-F4-02` is open and this refusal does not wait on it.** That question asks whether a provider would honour an **admin-attributed** verification at all, which is [ADR-069](../decisions/ADR-069.md)'s parity question and a vendor answer nobody has in writing. **The impersonation case is the opposite shape and is already decided**: the session is trader-attributed, so the assertion is not admin-attributed-and-possibly-worthless, it is **trader-attributed-and-false**. The two are separated here because a reader who finds one open question near one refusal tends to treat the refusal as provisional.

**Nothing about `INV-M19-07` changes, and it is why the refusal is narrower than it looks.** Merit never proxies a document: the client goes to the provider's hosted flow. **So a refused route is a refused route and not a refused ceremony**, and an operator who somehow held a hosted URL would be presenting their own face to a provider whose job is to notice that. The route is refused because it **writes**: it creates a verification, records a placement, and can move state.

**The contact-change refusal is `C-27`'s and this module adds nothing to it.** [SECURITY:45](../architecture/SECURITY.md) already refuses a contact change of either kind from any session that cannot elevate, and an impersonation session can never elevate because [`0042`](../../packages/db/migrations/0042_impersonation_sessions.sql) carries no `elevated_at` and no `elevated_by_factor`; those columns are `sessions`' ([`0029`](../../packages/db/migrations/0029_phone_identity_and_auth.sql)). **The six enforced legs of the phone-change ceremony are this module's and they are downstream of a route the session never reaches.** Writing a second refusal here would claim new enforcement where an old invariant already holds, which is [ADR-068](../decisions/ADR-068.md) section 1's whole finding, and `M6-N-05` pins the **outcome** so that the refusal does not vanish silently if the boundary ever moves.
...
**The edge case and golden scenarios are allocated and this citation is now live: [EC-143](../edge-cases/EC-143.md), GS-262 and GS-263.** They were deliberately unnamed here until the registries session wrote the rows, for the reason [FOLD-01 section 4](FOLD-01-phone-identity.md) records about the delta numbers: pre-claiming in a registry that has no claim mechanism is the drift [ADR-034](../decisions/ADR-034.md) and [ADR-036](../decisions/ADR-036.md) were built to stop, and `CI-06d` fails the build on a `GS-nnn` that does not resolve. **Two scenarios rather than one, because the residual is the half that gets forgotten**: GS-262 pins the guard working, and GS-263 pins the reassignment with **no portability record at all**, which ends in a review queue with nothing enforced and is the corpus `DEP-M19-09` exists to keep rare.
