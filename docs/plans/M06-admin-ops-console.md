...
| INV-M6-15 | **Open Liability does not move when a wallet withdrawal is approved, and it falls when that withdrawal's cash leaves** | [ADR-195](../decisions/ADR-195.md). `LT-06` moves `amount_cents` out of the trader's `trader_wallet` position and into the firm-scoped `withdrawals_in_flight` obligation ([ADR-187](../decisions/ADR-187.md), [`0056`](../../packages/db/migrations/0056_eighth_ledger_code.sql)), so a panel built from the wallet term alone falls at the **approval** and does not move at the **settlement**: it dates the fall to the wrong event and then never reports the right one. The obligation is `P-M6-01`'s **third component**, `LT-07` is the only external-leg posting that moves this panel, and `LT-09`'s reversal ([ADR-189](../decisions/ADR-189.md)) returns the amount to the wallet component without moving the total. **A figure beside the panel cannot satisfy this**, because the defect is in the total rather than in what is shown next to it |
...
|---|---|---|
| P-M6-01 | Open liability | `sum(withdrawable_cents)` across funded accounts **plus `sum(wallet_balance_cents)` across identities**, as of the last closed day (INV-M6-11). This is the **accounting** figure: what traders could claim if every gate vanished. **The two components are shown separately as well as summed**, because they behave differently: withdrawable is a claim that still has to clear gates, and a wallet balance has already cleared them all and is owed unconditionally. **AMENDED ADDITIVELY and every word above stands** ([ADR-195](../decisions/ADR-195.md)): there is a **third component**, the firm-scoped `withdrawals_in_flight` obligation ([ADR-187](../decisions/ADR-187.md)), which is what `amount_cents` becomes between `LT-06` and `LT-07`. It is shown separately and summed like the other two, and it behaves differently from both: it has cleared every gate **and been acted on**, and it is the only component with two exits, `LT-07` out as cash and `LT-09` back into the wallet component. `INV-M6-15` is the rule that follows. **The sentence above says two components because it was written before the eighth ledger code existed**, and it is kept rather than corrected on [ADR-180](../decisions/ADR-180.md)'s and [ADR-181](../decisions/ADR-181.md)'s precedent in [M05](M05-payout-system.md) section 2.1 |
...
| P-M6-04 | Payout velocity | Trailing 7 day settled cents against the 30 day average, alarming above 2.5x (constitution M6). **AMENDED ADDITIVELY and every word above stands** ([ADR-201](../decisions/ADR-201.md)): both windows are counted in **trading days**, every settled payout is attributed to the first trading day on or after its settlement date so none is ever dropped for landing on a day the exchange was shut, and `avg_30d_cents` is the trailing **30 trading days scaled to seven**, `floor(total_30_cents * 7 / 30)`. **That reading is proved rather than chosen**: of the three readings this row's "30 day average" admits, the 30-day total makes the alarm provably unable to fire, since the trailing 7 days are a subset of the trailing 30 and the ratio therefore never exceeds 10000 bp; and the mean daily figure puts steady state near 70000 bp, so the pager fires every day forever. **The threshold does not move and was never in doubt**: this row, [the constitution's M6 paragraph](../../MERIT_BUILD_MASTER_PROMPT.md) and [INFRA](../architecture/INFRA.md)'s alarm inventory state 2.5x identically and strictly, which is `25000` bp compared with `>`. **With an empty denominator `ratio_bp` is 0 and `alarm` is false**, and that entry's section 7 says plainly what it costs. The window is a **structural ruling**, not a plan parameter, not a config value an operator sets, and not a column |
...
| Endpoint | Obligation |
|---|---|
| `GET /admin/liability` | Response gains `open_liability_bounded_cents`, the identity-max eligible figure, and `absorbed_corrections_cents` (SD-M6-01). Every field carries its own `as_of` (INV-M6-04). **And the in-flight obligation** ([ADR-195](../decisions/ADR-195.md)), `P-M6-01`'s third component and `INV-M6-15`'s subject. **It is the one figure on this response with no column behind it**: no column of [`liability_snapshots`](../architecture/data-model/liability_snapshots.md) holds this obligation, so the field lands only with a migration, and its delta is the **single number between `SD-M6-07` and `SD-M6-09` that section 2 leaves deliberately unclaimed**, named by position there and here for the same reason. **Neither the migration number nor the delta number is taken by that entry** |
