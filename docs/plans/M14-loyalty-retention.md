...
|---|---|---|---|
| LM-M14-01 | **Progressive cap release** | A higher payout cap from a configured ordinal onward, using `payout_cap_schedule`'s existing array shape | **REJECTED for v1** ([ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted)). Rejected rather than deferred. The array shape stays; no second step is published. AS-M14-01 |
...
| LM-M14-05 | **Cross-account loyalty** | The **Nth completed ladder** across an identity's accounts earns account-spanning perks: reset discounts, bonus credit, review-pool priority | **Ship. This is the replacement for LM-M14-01** ([ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted)). **No per-account bound moves.** AS-M14-08 |
...
| INV-M14-10 | Loyalty credit is `promotional_credit` in the ledger, never `trader_wallet` | [ADR-019](../DECISIONS.md) activated both classes and they are not interchangeable. Wallet balance is money the trader earned and can withdraw; promotional credit is a discount instrument that is spendable and **never withdrawable** (AS-M14-03). **[ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted)'s "bonus credit" perk is issued under this invariant**, rendered inside the wallet screen next to the withdrawable position and labeled as the separate thing it is |
| INV-M14-11 | **No loyalty mechanic moves a per-account bound.** `payout_cap_cents`, `max_payouts`, `profit_split_bp`, `cadence_gap_trading_days`, and `win_days.required_count` are the same for a first-time buyer and for an identity on its tenth completed ladder | [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted). This is INV-M14-01 stated as an outcome rather than as a permission, because INV-M14-01 is satisfied by a plan-version publish and this one is not. [M01](M01-rules-engine.md) INV-17's lifetime bound is identical on every account Merit ever sells at a given plan and size |
...
**`ladders_completed_lifetime` is the only column [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted) required, and it is derived like everything else here.** A completed ladder is an account whose phase is `graduated` with `payouts_settled_count = max_payouts`, both of which the engine already writes, so the counter is a `count(*)` over an existing fact rather than a new piece of state anybody can set. That is what keeps LM-M14-05 inside INV-M14-03: the loyalty program's trigger is a thing the money path produced, not a thing the loyalty service decided.

**Two reservations used rather than added, and one now deliberately unused.** `promotional_credit` is already a ledger account class ([ADR-019](../DECISIONS.md)) and carries LM-M14-05's bonus credit. `payout_cap_schedule` is already an array in the plan config ([DATA_MODEL section 12](../architecture/DATA_MODEL.md), which names progressive cap release as the reason) and **stays an array with exactly one step in v1**: [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted) rejected the second step, not the shape. Keeping the shape costs nothing, keeps the publish-time validations exercised, and means a future founder who overrules the rejection is making a config decision through the dual-controlled publish path rather than a migration decision on a money-path table. This module still needs no new money path.
...
**This section replaces the progressive-cap-release sequence that stood here** ([ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted)). The path that mattered about that mechanic, which is that a cap change is a dual-controlled plan-version publish and nothing else, is preserved in section 3.5 as a **negative** flow, because it is now the thing the module must be provably unable to do rather than the thing it does.
...
Progressive cap release is rejected ([ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted)), so the sequence below is documented as **the only route a cap could ever change by, and the route this module is provably unable to walk**. It is kept rather than deleted for two reasons: a future founder who overrules the rejection needs the path written down, and the negative tests in section 8.1 assert against it by name.
...
**And `POST /admin/plans/:planId/versions` is no longer in this table.** It was here as the cap-release proposal path. [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted) rejected the mechanic, so **M14 now touches the plan-publish surface at no verb, in no direction**, which is a stronger position than proposing through it and a much easier one to test: the negative suite asserts the loyalty service reaches that route not at all, rather than asserting it reaches it only politely.
...
**`loyalty.cap_release_proposed` is retired unpublished.** It carried `{ plan_code, from_ordinal, proposed_cap_bp, cohort_size, projected_lifetime_delta_cents }` and existed so the founder could never see a cap-release proposal without its liability delta. [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted) removed the proposal, so the event has nothing to announce. It is recorded here as retired rather than deleted from the plan, because an event name that once existed in a design doc gets re-implemented by somebody reading an old branch, and the note is cheaper than the incident.

**Consumed:** `wallet.credited` (the payout ordinal that drives most criteria, per [ADR-019](../DECISIONS.md)), `breach.detected`, `account.provisioned`, `purchase.charged_back`, `flag.status_changed`, `day.closed`, and `account.graduated` (**now load bearing**: it is LM-M14-05's trigger rather than a background fact).
...
| FM-M14-11 | Review-pool priority is read as an entitlement to a live invitation | Merit is understood to have promised the thing [ADR-024](../DECISIONS.md) exists to have not promised | Copy lint on every surface that renders loyalty state, sharing [M18](M18-graduation-track.md)'s live-program lint | Priority is an ordering key with no trader-facing rendering in v1 (INV-M14-12). AS-M14-08 |
...
**Disposition, first, because the rest of this scenario is now the argument for a decision rather than a proposal.** [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted): **progressive cap release does not ship in v1, and it is rejected rather than deferred.** LM-M14-05 replaces it. The analysis below is what the ruling was made on and it stands unchanged; only the counter has changed, from a process to a prohibition.
...
1. **The mechanic is rejected for v1** ([ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted)), and rejected rather than deferred, so it returns as an ADR arguing against a number rather than as a roadmap item nobody re-prices.
...
4. **The path stays documented as a negative flow** (section 3.5), because a rejected mechanic whose only route was deleted is a rejected mechanic somebody rebuilds badly. **The ladder length remains structural** and the cap's value remains a config, per the [parameter-status ruling](../DECISIONS.md#parameter-status-launch-candidates-versus-structural-rulings-founder-ruling-2026-08-14). EC-104, GS-179, GS-243.
...
- **Review-pool priority puts the ring at the front of a discretionary queue.** The queue exists to decide who is looked at for a live invitation. Ordering it by completed ladders means ordering it by the metric a ring optimizes, which would hand the adversary the one thing the discretion in [ADR-024](../DECISIONS.md) was designed to withhold.

**What this scenario is emphatically not.** It is **not** an argument that the mechanic moves liability. It does not: INV-M14-11 holds and INV-17's bound is identical for every identity at every tier, which is the whole reason [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted) chose it. The worst case here is a **bounded marketing loss plus a mis-ordered queue**, which is a materially different object from a 40 percent increase in what an undetected ring may extract. Recording that comparison is the point of the scenario, because the temptation at review is to treat "rings can farm this too" as though it were the same finding, and it is not.
...
| A `payout_cap_schedule` with more than one step reaches publish validation | any | **page**. [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted) is a v1 rejection, so this is not an error condition; it is a decision that must not happen by accident |
...
**OQ-M14-06. NEW. What is N, and what does each tier of it pay?** LM-M14-05 needs a schedule: which completed-ladder count earns which perk, at what value. Proposed as a launch candidate under the [parameter-status ruling](../DECISIONS.md#parameter-status-launch-candidates-versus-structural-rulings-founder-ruling-2026-08-14), since every value here is a `loyalty_criteria` row rather than an engine input: **N = 1** earns a reset discount, **N = 2** earns bonus credit, **N = 3** adds review-pool priority, with the perk budget capped per cohort. The values matter less than that the schedule is published before anyone can reach it (INV-M14-07), and the founder should set the budget ceiling rather than the perk values, because the ceiling is the control and the values are marketing.

**OQ-M14-07. NEW, and it is the one carrying a divergence.** [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted) names the perk "bonus wallet credit". **This plan implements it as `promotional_credit` rendered inside the wallet screen, spendable on any Merit product and never withdrawable**, because the literal reading breaches INV-M14-10, [M20](M20-wallet.md) INV-M20-03 and INV-M20-11, `INV-WALLET-NO-DEPOSITS`, and [M17](M17-offers-engine.md) INV-M17-08, and because it would hand an attacker a laundering path that does not require passing an evaluation. **Confirm or overrule at the FREEZE gate.** Overruling is a money-path change to a closed check constraint, needs its own ADR and its own session under [ADR-003](../DECISIONS.md)'s strict regime, and requires re-running [AS-M20-01](M20-wallet.md)'s conversion economics.
...
| DEP-M14-07 | The simulation harness can price a cap-schedule change as a CVaR99 input | Wave 4 | Retained after [ADR-025](../DECISIONS.md#adr-025-progressive-cap-release-is-rejected-for-v1-and-replaced-with-cross-account-loyalty--2026-08-14-status-accepted): the rejection is a v1 decision, and any future reversal needs the number before it needs a plan |
| DEP-M14-08 | M18 exposes the graduation-eligible review pool with an **orderable** key, and never reads loyalty state as a membership condition | M18 | INV-M14-12 collapses into an entitlement, which is exactly the promise [ADR-024](../DECISIONS.md) decoupled the ladder from |
