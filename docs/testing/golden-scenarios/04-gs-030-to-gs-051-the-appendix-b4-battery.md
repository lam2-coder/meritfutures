...
|---|---|---|---|---|
| GS-030 | 1 | DST transition day | M1 | **Amended by [ADR-077](../../decisions/ADR-077.md), which struck a falsified arithmetic and left every obligation in place.** Day boundary follows the exchange calendar. ~~The 23 hour and 25 hour sessions~~ **The sessions either side of a DST transition** each produce exactly one trading day and one mark, **and their identical 16:00 CT close resolves to UTC instants an hour apart**. **The struck clause is kept rather than deleted** ([ADR-052](../../decisions/ADR-052.md)'s idiom at [M01:1150](../../plans/M01-rules-engine.md)): three documents recited it, so a reader arriving from one of the others has to be able to land here and see what happened. **There is no 25 hour session and there never was**; the transcribed CME session rule is 17:00 CT to 16:00 CT, 23 hours every day of the year, and both transitions fall at 02:00 CT on a Sunday inside the weekend gap, so no session contains one and no session's length changes. **The DST hour is real and lands in that gap, 48 hours on a spring-forward weekend and 50 on a fall-back one against an ordinary 49**; those numbers stay in the ADR and out of this row, because nothing executes them. **"and one mark" is TRUE and UNASSERTED** and is deliberately not dropped: `DailyMark` carries no instant and `EngineInput` no calendar. The count half IS asserted, over all six transitions, at [`trading-calendar-generator.test.ts`](../../../packages/db/test/trading-calendar-generator.test.ts) |
