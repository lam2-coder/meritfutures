...
| ID | Scenario | Pins |
|---|---|---|
| GS-285 | A Tier-1 event renders on two traders' dashboards in two timezones | Both read **one** `economic_calendar` row. The panel's source is Merit's table and not an embed, which is the only mechanical form of "one source of truth for when was the news" |
| GS-286 | A scheduled release time is revised after publication | The panel and `D-04`'s window **both** move to the new instant. `DEP-M7-06` is satisfied by a dataset that carries a revision, not by a static import |
| GS-287 | The calendar passes its staleness threshold | The alarm fires per `FM-M7-08`, and **`D-04` declines to fire rather than firing on stale windows**. Firing on a wrong window manufactures evidence against a trader, which is worse than not firing |
