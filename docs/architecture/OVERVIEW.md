...
graph TB
    ...
    Admin --> API
    API --> DB
    API -.->|live frames, indicative| Portal
    API -.->|live frames, indicative| Admin
    Worker --> DB

    Worker <-->|SFTP CSV out, reports in| Rithmic
    Rithmic -.->|tier 2, indicative ticks| Worker
    API -->|checkout| PSP
...
**The three dotted edges are tier 2 and the notation is the whole of what it means** ([ADR-020](../decisions/ADR-020.md), [ADR-163](../decisions/ADR-163.md)). A solid edge is a request somebody made and an answer that is authoritative; a dotted edge is delivery nobody asked for, carrying a value labelled indicative on the number itself (`INV-M4-11`). The feed reaches `Worker` because streaming ingest is **write-only into the live cache** (`INV-M2-14`), and the frames leave `API` because the live channel is served by the same deployable and the same startup route-set choice that produces the operator 404 ([ADR-083](../decisions/ADR-083.md)); the worker has no ingress and never serves the channel.

**THE LEG BETWEEN THEM IS DELIBERATELY NOT DRAWN.** The live cache is a store whose medium is an open ruling, and two of the three readings available to it draw no node here at all: a table inside `DB`, or a process-local store inside `API`. Drawing one would settle that ruling in a diagram, so the edge from the ingest to the channel arrives with the medium and not before it ([ADR-163](../decisions/ADR-163.md) clause 5).
...
| `apps/api` | Every endpoint [API_CONTRACT](API_CONTRACT.md) specifies, under the `/api/v1` base path: auth, catalog, commerce, accounts, KYC and affiliate, the operator and `/internal/*` surfaces, and the signed inbound webhooks, and **[ADR-020](../decisions/ADR-020.md)'s tier-2 live channel, on both deployments** ([ADR-163](../decisions/ADR-163.md)) | **Drawn in section 2 since 2026-08-13 and rowed here only on 2026-08-23**, by [ADR-083](../decisions/ADR-083.md), which is the ruling that created the deployable. It is separate from the three UI surfaces because API_CONTRACT section 1 makes them its **clients** with "no privileged back door", and a surface that contains the API has one by construction: a handler in the same package is an import away. **One codebase, two deployments.** The public one registers no `/admin/*` and no `/internal/*` route, so section 12's required **404** from the public origin is the router answering for a path that is not there rather than a per-request check that can be forgotten. **The live channel is here for that same sentence rather than for convenience**: what withholds it from the public deployment is the startup route-set choice and nothing else, so a socket served by any other process would be withheld by a second copy of the prefix rule or by nothing at all ([ADR-163](../decisions/ADR-163.md)) |
...
| `packages/rithmic` | Platform adapter: provision, entitle, ingestFills, ingestEOD, reconcile, and **`streamLive`, ADR-020's tier-2 indicative feed** ([ADR-154](../decisions/ADR-154.md)). Plus the synthetic simulator, which produces the report files the first five consume and **implements `streamLive` and nothing else** | Isolates every vendor specific behind the interface so adapter #2 is additive. **In tier 1 the simulator and the vendor share an ARTIFACT** (one CSV into one ingest path, one parser, `INV-M2-11`); **in tier 2 there is no file to share, so what they share is the TYPE** `LiveAccountTick`, which carries `indicative: true` as a required literal so no consumer can destructure a tick without meeting the word |
| `worker` | Nightly batch, provisioning delivery, Rise transfers, detectors, hygiene jobs, and the **streaming ingest**, which is write-only into the live cache and reads the adapter's `streamLive` ([ADR-020](../decisions/ADR-020.md), `INV-M2-14`) | Long-running and retryable work must never sit in a request path. **It holds the tier-2 INGEST and never the tier-2 CHANNEL** ([ADR-163](../decisions/ADR-163.md)): serving the socket here would need ingress on the one service [INFRA section 2.1](INFRA.md) rows with none, and would put the trader's audience and the operator's in one process with no `MERIT_API_SURFACE` between them |

**Tier 2 adds no container and this table gains no row, which is a ruling rather than an omission** ([ADR-163](../decisions/ADR-163.md)). The live channel is `apps/api`, the ingest is `worker`, the adapter is `packages/rithmic`, and the live cache's medium is an open ruling whose own row lands with it. A live tier given a container of its own would be a sixth deployable, and this table's column is *"Why separate"*: `RI-04` and `RI-09` both iterate a hand-written list of five, so a sixth application directory is invisible to the two checks written to see exactly the failure a sixth would introduce.
