...
**Payloads carry ids and numbers, never PII.** No email addresses, no names, no document data, no card details, no IP addresses inside a payload. An event references `identity_id` and `account_id`; anything sensitive is fetched through an authorized read at display time. This keeps the forever-retained table free of the data a privacy deletion request would have to reach into. **An event that references neither is a fact about the FIRM, and it says so on its own row with the mark `**FIRM**` after its name** ([ADR-205](../decisions/ADR-205.md)); an unmarked row reaching no identity and no account is a producer defect and never a firm fact. Section 14 is the mark, its one holder, and the reason.
...
| `detector.run_completed` **FIRM** | Worker | `{ detector_run_id, detector, detector_version, trading_day, rows_scanned, flags_raised, duration_ms }` | BI, ALERT on failure |
...
## 14. Tenancy in a row: the `**FIRM**` mark

**Added by [ADR-205](../decisions/ADR-205.md), which rules it. Two lines above this section changed and no line MOVED**: the tenancy sentence in section 1 and `detector.run_completed`'s row in section 8 were each rewritten in place, at the same line count, because 85 live citations across 18 files reach into this document by line number and 31 of them sit under `apps/worker/src`, which no fence holds today. Every other field list, consumer cell and `schema_version` in this catalogue is byte for byte as it was.

**[`events`](data-model/events.md) declares `identity_id` and `account_id` both nullable and ties them with no CHECK**, and [ADR-191](../decisions/ADR-191.md) registers the table under the scope class `either`, whose predicate is the DISJUNCTION of the two. So a scoped read returns this person's events by either path **and the firm's rows by neither**, which is that entry's own sentence about this table. A row reaching neither column is therefore invisible to every per-trader read of a table that is append-only and retained forever, and nothing about the absence distinguishes it from the event never having happened.

**That is correct for a fact about the firm and catastrophic for a fact about a trader, and the two are not distinguishable from the row.** So the catalogue distinguishes them, in the one place a producer may read: **a row whose fact belongs to the firm rather than to any trader carries `**FIRM**` after its event name.** A producer writes an untenanted `events` row for a marked name and refuses one for every other name, and it invents the mark for nothing.

**The consumer legend cannot decide this and the attempt is refused on evidence.** The tempting rule is that a row with no `TL` consumer is firm-level, `TL` being section 2's per-account view. It is wrong on the row this corpus has already ruled: `payout.freeze_expiring` at `EVENTS:255` is consumed by `ALERT, FEED` and by no `TL`, and it is a fact about ONE trader's frozen payout whose payload merely fails to name a column. [ADR-191](../decisions/ADR-191.md) rules that one a **producer** defect, to be repaired by resolving the tenancy at write time from the `payout_request_id` the producer holds. A derivation that admitted it would write that trader's warning as a firm row forever, so the mark is explicit, per row, and never inferred.

**The mark is not a licence to leave a tenancy unresolved.** A row qualifies only where the fact belongs to no identity and no account **and there is no correct one to resolve** -- not where resolving one is merely inconvenient. `detector.run_completed` qualifies because [`detector_runs`](../../packages/db/migrations/0008_risk.sql) declares no column against `identities(id)` and a nightly sweep over the whole population has no owner; the accounts it disagreed with are its OUTPUT, recorded on `risk_flags`, rather than its subject. [`0064_reconciliation_runs.sql`](../../packages/db/migrations/0064_reconciliation_runs.sql) states the same test for the same shape and names `detector_runs` while doing it.

**`detector.run_completed` is the only row marked today, and the mark sweeps nothing.** Thirty-three other readable rows in this catalogue name neither tenancy column, and each stays refused until its own fence establishes, row by row, which of the two cases it is. An unmarked row is not thereby a trader's; it is undecided, and a producer refusing it is the honest reading of an undecided row.

**A marked row still fills `subject_kind` and `subject_id`, which are `NOT NULL` for every row including the firm ones.** That is why `detector_run_id` joins the payload above: `events.subject_id` is a `uuid NOT NULL` at [`0017:47`](../../packages/db/migrations/0017_events_and_audit.sql), the run has a real primary key, and the mark answers tenancy alone. **`schema_version` does not move**, on section 13's argument and for section 13's reason: no row of this name has ever been written, so there is no earlier shape to keep readable.
