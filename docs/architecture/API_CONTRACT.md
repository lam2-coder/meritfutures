...
**Cross-origin writes are refused.** Every request whose method is not `GET`, `HEAD` or `OPTIONS` is checked before its body is parsed: if it carries an `Origin` header naming a hostname other than the one it was sent to, it is refused `403 forbidden` and no handler runs. This is the constitution's `CSRF on cookie mutations` and it is [C-29](SECURITY.md#1-control-catalogue) ([ADR-221](../decisions/ADR-221.md)). **There is no token and no endpoint that issues one.** A caller that sends no `Origin` at all is admitted, which is every server-side caller including the portal and every `/webhooks/*` sender; a browser page served from this API's own hostname is admitted because its `Origin` matches. **A browser on any other hostname cannot call a mutating endpoint**, and that is deliberate rather than incidental: `SameSite=Lax` on the session cookie is a SITE control, and the marketing site shares a site with this API. The refusal names no reason beyond the canonical `forbidden`.
...
**`failure_note` IS `null` ON EVERY ROW v1 SERVES, AND THAT IS A RULING RATHER THAN AN OMISSION** ([ADR-290](../decisions/ADR-290.md)). No column holds it, which the enforcement endpoint's own note below (`POST /admin/payouts/:id/enforce`) already recorded and this ruling now resolves. The field describes `status: "failed"` and nothing else, and the only two edges into `failed` are `G-HOLD-ENFORCED` and `G-FREEZE-ENFORCED` ([STATE_MACHINES](STATE_MACHINES.md) section 2), both enforcement acts. **`payout.transfer_failed` never reaches this field**: rail exhaustion belongs to `wallet_withdrawals` on the external leg, so the truthful note [M04](../plans/M04-trader-portal.md) section 5 requires for a failed transfer is a different note on a different record. **Two things are owed before the field carries a value and neither is `GET /payouts`'s**: a `payout_requests.failure_note` column, specified in [ADR-290](../decisions/ADR-290.md) section 2.4, and a field on `PayoutEnforceRequest` to supply it, since `reason` is admin reasoning and this section excludes admin reasoning from every trader projection. **The declared type does not move**, because `null` is already in it and narrowing it would make every future producer an amendment.

**`timeline` IS `[]` ON EVERY ROW v1 SERVES, AND ITS SOURCE IS NAMED** ([ADR-290](../decisions/ADR-290.md)). The source is [events](EVENTS.md), addressed on `subject_kind = "payout_request"` and `subject_id = payout_requests.id`. **It is empty because no deployable writes an event yet**, so `[]` is the projection of that relation rather than a placeholder standing in for one. **A timeline folded from `payout_requests`'s own state timestamps is REFUSED**: the hold-completeness and freeze-completeness constraints are biconditionals on status, so `held_at` and `frozen_at` are NULLed the instant the row leaves those states, and the fold would render a settled payout that was held for forty hours with no hold in it. A partial history that reads as a complete one is worse than an empty one, and the trader it hides the hold from is the trader who was held. **This is not `GET /accounts/:accountId/timeline`**, whose item is `{ occurred_at, trading_day, kind, summary, detail }`, is keyed on the account rather than the request, and must not be reached for as this field under another name.
...
**This row is [ADR-263](../decisions/ADR-263.md)'s amendment, and the paragraph it replaces said the opposite.** Until 2026-08-30 the text here read: *"There is no endpoint that cancels a withdrawal. `G-TRADER-CANCELS` is drawn from both `requested` and `cooling` in [STATE_MACHINES section 3.2](STATE_MACHINES.md) and `cancelled` is in `wallet_withdrawal_status`, and no approved document states a route, a body or an error set for it. It is named here as owed rather than invented, on [ADR-113](../decisions/ADR-113.md) clause 5's precedent for the operator half it could not write."* **It is quoted rather than deleted, because a sentence removed leaves nothing for a later reader to check.**

**What moved is that the route stopped being unwritable.** [ADR-234](../decisions/ADR-234.md) built `requested --> cancelled` and `cooling --> cancelled` under `G-TRADER-CANCELS`, guarded, with [`0072`](../../packages/db/migrations/0072_terminal_withdrawal_transitions.sql) binding the arrow set at the database, and left the transition with no caller. **`cancelled` is one of the three statuses that RELEASE an identity**, and every other status a created withdrawal can hold is inside `wallet_withdrawals_open_idx`'s predicate, so with the edge built and no door a trader opens a withdrawal they cannot close and `G-NO-IN-FLIGHT` refuses every later one, permanently. **The body and the error set below are what was owed; they are stated here and are not invented by the module that serves them.**

```ts
// No request body. The id is the path segment and there is nothing else to say.
type WithdrawalCancellation = {
  withdrawal_id: string;
  status: "cancelled";            // the only status this door answers 200 on
  cancelled_at: string;           // wallet_withdrawals.cancelled_at, added by 0072
};
```
Auth: **session**, and **elevated**: `passkey or dual_channel`, **carrying no `C-27:` tag**. C-27's action list is closed at payout destination change, contact change and external withdrawal, and a cancellation is money staying rather than money leaving, so tagging it would record that this door performs the act C-27 guards. **The factor is required on the resource rather than on the act**: a session that could not open this row may not close it. It costs a trader nothing they have not already paid, because the row exists only because an elevated session created it, and a trader who cannot elevate cannot open the next withdrawal either. Idempotency: **not required**, and section 1's required set is unchanged; the header is accepted and unread, because this door writes no row a retry could duplicate and a second cancellation is refused by the state rather than by a key. Rate limit: **none stated, and it is named as owed rather than invented**: section 11 rows a limit per endpoint where one is ruled and carries no authenticated-write class, the `POST /wallet/withdrawals` row above states none either, and a number chosen here would be a parameter this document invented. A cancellation writes one row of the caller's own and opens nothing, so the exposure a limit would bound is a trader cancelling their own request repeatedly. Errors: `conflict` (the withdrawal is not cancellable, or a halt is live on it), `forbidden` (session not elevated), `unauthenticated`.

**`conflict` is one byte-identical document for three different situations and that is the control rather than an imprecision.** An id naming no row, an id naming another identity's row, and the caller's own row at a status `G-TRADER-CANCELS` is not drawn from all answer the same refusal, which names the machine and never the row: section 13's ruling 1 is that existence is not confirmed to a stranger, and a detail naming the current status would answer the first two apart from the third. **The halt is the one refusal that names itself**, because section 6.2 already renders the halt to the trader on a subsequent read, so saying an investigation is open discloses nothing this surface withholds.

**A cancellation carries no guard except the halt, and the absences are the specification** ([ADR-234](../decisions/ADR-234.md) section 3). There is no KYC term, no destination term, no identity-status term, no `payouts_frozen` term and no dual control at any amount: every term on the approval edge guards a movement, and a cancellation moves nothing. **The phone-change hold does not reach it either**, for the same reason: `withdrawal_hold_until` exists to stop money leaving after a contact change, and this door stops money leaving.
