...
| C-29 | **Cross-origin refusal on every unsafe method.** A request whose `Origin` header names a hostname that is not the one it was sent to is refused `403` before its body is parsed, on every method except `GET`, `HEAD` and `OPTIONS`, on both deployments. This is Appendix D section D2's *"CSRF on cookie mutations"* and **`SameSite=Lax` was never it**: `SameSite` is scoped to the SITE, [INFRA section 2.1](INFRA.md) rows the marketing site and the API on hostnames that share one registrable domain, and a `Lax` cookie is therefore attached to a form `POST` issued from the marketing origin. The comparison is on HOSTNAME because the session cookie is host-only, so that is exactly the boundary the cookie draws | [ADR-221](../decisions/ADR-221.md). [`apps/api/src/csrf.ts`](../../apps/api/src/csrf.ts) decides and [`server.ts`](../../apps/api/src/server.ts) wires it as a root `onRequest` hook, so its coverage is the instance's rather than a list. §2.1, §2.6, [API_CONTRACT §1](API_CONTRACT.md#1-conventions) |
...
| S | **Script on the marketing origin issues `POST /wallet/withdrawals` in a signed-in trader's browser.** The two hostnames share a site, so the session cookie is attached and the request arrives fully authenticated | **C-29**, which compares hostnames rather than sites and refuses. C-27 bounds what a non-elevated session could have reached even if it did not |
...
| D | **Merit itself holds a payout indefinitely.** The adversary is the firm under pressure, and every individual step is defensible: there is a flag, a reason, a ToS clause, and an investigation that is ongoing | §4 items 6 and 8. **The hold is born with its clock** (`payout_requests_hold_is_complete`), expiry **pays**, there is no extend endpoint, and the releaser's failure is watched on the query as well as on the job, unsuppressibly. This row is in a threat table on purpose: **a denial nobody had to authorize is still a denial**, and the zero-denial policy was enforced against the explicit kind and silent about this one |
...
| E | **Login forgery**: a forged sign-in `POST` lands the attacker's session in the victim's browser, and everything the victim does next lands in the attacker's account | **C-29**. It is checked on the METHOD and never on the presence of a cookie, so a forged write that carries no session yet is refused on the same clause as one that carries a stolen one |
...
6. **The freeze contract:** a freeze requires at least one cited open flag and a ToS clause. This is enforced by the API, not by discipline, because the pressure to "just hold this one" arrives exactly when the money is largest. **Extended by [ADR-040](../decisions/ADR-040.md): the contract now includes a clock, and the clock is enforced by the schema rather than by the API.** `payout_requests_hold_is_complete` ([`0031`](../../packages/db/migrations/0031_payout_hold_and_identity_restriction.sql)) makes a hold carrying a flag but no `hold_expires_at` **unwritable**, which is a stronger place to put the guarantee than a handler: the pressure item 6 names arrives at the handler, and a constraint is not on call.
...
8. **The hold contract, and the reason it is in this section at all** ([ADR-040](../decisions/ADR-040.md)). `held_pending_review` is a **pre-approval** state entered when an unresolved severity 4+ flag stands at request time, and it is a security control that **runs against Merit rather than against a trader**: the hold either pays inside **48 wall-clock hours** or produces a documented enforcement action carrying a cited flag, a ToS clause and an evidence pack. Four properties are enforced rather than intended, and the fourth is the one this section exists for.
   - **A hold cannot be written without its flag, its clause, its written reason and its clock**, all five columns or none (`payout_requests_hold_is_complete`).
   - **A held request has posted nothing** (`INV-M5-21`), so an enforcement reverses nothing and a release posts the **stored** decision unchanged, re-evaluating no gate.
   - **There is no extend endpoint** ([M05 section 4](../plans/M05-payout-system.md)). The clock is the control, so an operation that moves it is the control's own off switch.
   - **The auto-release is a job, and a job can stall where a constraint cannot.** So it carries an **S1 dead-man switch** on the sweep ([CRON_INVENTORY](../ops/runbooks/CRON_INVENTORY.md)) **and** a nightly assertion that fires on **the query** rather than on the job's own report, because a job that reports success is not evidence that the work happened. **Both are unsuppressible** (`INV-M5-17`), which makes this the **fourth** entry on that list, amending [M06](../plans/M06-admin-ops-console.md) `OQ-M6-01`. The other three mean Merit cannot safely pay; **this one means Merit has stopped paying and nobody is being told**, and unlike the vendor-gap alarm OQ-M6-01 declined, the condition here is resolvable by Merit alone, in one action, with no third party in it.
9. **The external leg's halt is enforcement, not a state** ([ADR-040](../decisions/ADR-040.md)). `wallet_withdrawals` carried `frozen_at`, `freeze_flag_id` and `freeze_expires_at` from [`0011`](../../packages/db/migrations/0011_wallet.sql) and `wallet_withdrawal_status` never gained a frozen value, so **a halted withdrawal still matched the open index and nothing refused settlement: the halt was representable and unenforced, in a shipped control, for the whole life of the table.** `wallet_withdrawals_live_freeze_blocks_settlement` is the enforcement, the open index is re-created so a halted row **stays visible**, and the halt is a CHECK rather than a status because it is orthogonal to the rail state. **The transferable lesson is item 9's, not item 9's table's**: columns that record a control are not the control, and the interval between writing them and enforcing them is not visible from either end.
10. **Identity restriction is the enforcement shape between a freeze and a closure** ([ADR-041](../decisions/ADR-041.md)). Reversible, **per human**, halting every linked account at once: payout requests blocked at `G-ELIGIBLE`, wallet spend and external withdrawal blocked at `INV-M20-06`, purchases and resets refused server side at checkout, affiliate settlement blocked because [ADR-017](../decisions/ADR-017.md) put every outbound payment on one rail and **a restriction that stops one door and not the other is not a restriction**, and platform trading revoked through the Rithmic bridge. **Account state is preserved intact**, which is what makes it reversible rather than terminal. Three security properties:
    - **The episode is a row, never a column.** `identity_restriction_episodes` ([`0031`](../../packages/db/migrations/0031_payout_hold_and_identity_restriction.sql)), because `identities` carries only `status` and `status_reason`, so a repeat restriction would overwrite its predecessor and **a restore would be unprovable at exactly the moment it is contested**. `identity_restriction_restore_is_complete` makes a restore with no actor or no evidence unwritable, and `identity_restriction_open_uq` permits at most one open episode per human.
    - **Both directions are fail-closed.** Out: `disable_entitlement`, then `disable_account`. Back: `set_risk` at the account's current floor **confirmed first**, then entitlement, then permissions, because re-enabling an entitlement against an unconfirmed setpoint is an unenforced funded account (`INV-M2-13`). **Restoration is PROVISIONAL under [ADR-005](../decisions/ADR-005.md) and contingent on `V-M2-15`**, and the honest form of that is an asymmetry rather than a caveat: **restriction is always available, restoration is not.**
    - **It cannot become a route around item 8.** Where a payout is held, the episode's `sla_due_at` binds the **restriction**, so a restriction cannot hold a held payout past its own 48 hours. That is asserted, not intended.
8. **The hold contract, which binds Merit rather than the trader** ([ADR-040](../decisions/ADR-040.md)). A `held_pending_review` request requires a cited open flag, a ToS clause, a written reason **and an expiry**, all five columns together or none, enforced by `payout_requests_hold_is_complete` in the database rather than by the API alone. **The expiry is the security control, not the flag.** A hold with a citation and no clock is an indefinite hold, which is a denial nobody had to authorize and which no audit would ever surface as one, because every individual hold looks justified.
9. **The releaser is a control, so a stalled releaser is an incident.** The hold joins the existing hourly freeze-expiry sweep and its **S1 dead-man switch**. Separately, a nightly assertion runs **on the query** rather than on the job: no request may sit past its hold or freeze expiry, evaluated independently of whether the sweep reported success ([M05](../plans/M05-payout-system.md) INV-M5-18). It is the **fourth unsuppressible alarm**, and the reason it cannot be muted is the reason it exists: **an alarm that can be muted is not a control**, and the pressure to mute this one arrives from inside the company.
10. **The external leg's halt is enforced, not merely representable.** `wallet_withdrawals` carried `frozen_at`, `freeze_flag_id`, `freeze_expires_at` and a freeze-expiry index from `0011`, and `wallet_withdrawal_status` has never had a frozen value: **a halted withdrawal still matched the open index and nothing refused settlement.** `0031`'s `wallet_withdrawals_live_freeze_blocks_settlement` is the enforcement. The general lesson is worth more than the fix: **a control that exists as columns and not as a constraint reads as done in every review and stops nothing at runtime.**
11. **Identity restriction is per human and halts every door at once** ([ADR-041](../decisions/ADR-041.md)). `identities.status = 'restricted'` blocks checkout, payout requests, wallet spend, external withdrawal and **affiliate settlement**, because ADR-017 put every outbound payment on one rail and a restriction that stops one door and not the other is not a restriction. It is a layer over the account machine: no account status moves and no ladder rung is consumed. **Restoration is a documented restore** carrying its actor and its evidence, and on the platform leg it is fail-closed: `set_risk` confirmed at the current floor **first**, then entitlement, then permissions, because re-enabling an entitlement against an unconfirmed setpoint is an unenforced funded account (INV-M2-13). The platform leg is **PROVISIONAL** under [ADR-005](../decisions/ADR-005.md), and the honest form is the asymmetry: **suspension is always available, restoration is contingent on `V-M2-15`.**
12. **Every restriction is an episode row, because a restore must be provable when it is contested.** `identities` carries `status` and `status_reason` and nothing else, so a repeat restriction would overwrite its predecessor. [`identity_restriction_episodes`](data-model/identity_restriction_episodes.md) records the citation, the clause, the actor, both clocks and the restore evidence, with at most one open episode per identity. **The 48 hour SLA binds the restriction rather than the payout**, so a restriction cannot hold a held payout past its own window, which is what stops the identity control from becoming a route around the payout control.
...
**Security events that always alert:** admin login, failed-auth burst (per IP and global), any `/admin/plans/*` mutation, role grant or change, payout-destination change, `payout.name_mismatch_detected`, `ledger.invariant_violated`, `replay.divergence_detected`, `recon.mismatch_detected`, SFTP delivery failure, webhook signature failure spike, canary-token access, **phone change applied**, **a number observed live on a second identity** (C-27's sibling in [M19](../plans/M19-kyc-identity.md) `INV-M19-13`), **every state change of the OTP cost breaker: the trip, the degraded window while it runs, and the recovery** (C-28), **any hold or freeze observed past its expiry** and **the hourly release sweep failing to run** (§4 item 8, both unsuppressible), and **an identity restriction opened or restored** (§4 item 10).

**The restriction alerts on both directions and that is not symmetry for its own sake.** An opening is an enforcement against a human across every account they hold; a restore **returns platform access**, and it runs through fail-closed provisioning that can leave an account entitled against an unconfirmed setpoint if any leg is skipped. **The direction that gives access back deserves at least as much attention as the one that takes it**, which is the same reasoning §4 item 5 already applies to `unfreeze`.
