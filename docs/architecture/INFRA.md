...
- **The OTP delivery credentials are NAMED here and VALUED nowhere ([ADR-229](../decisions/ADR-229.md)).** `MERIT_POSTMARK_SERVER_TOKEN` is the mail vendor's **server** token, which sends from one server and cannot manage the account, the domains or the other servers; the account token that can is a different credential and no Merit service is given one. `MERIT_OTP_EMAIL_FROM` is the sending address. Both are set on the **`api`** service and on no other, per the per-service scope above. **Absent either, `POST /auth/otp` answers 503 rather than pretending to send**, which is [ADR-226](../decisions/ADR-226.md)'s rule that an absent secret is a refusal and never a disabled control, applied to a capability instead of to a check.
- **`MERIT_OTP_SMS_PRICE_CENTS` is named and is set nowhere, and the absence is deliberate ([ADR-229](../decisions/ADR-229.md)).** It is the per-send price an SMS charges against `otp_send_budget.spend_cents`, in **integer cents, rounded UP by whoever sets it**: a fraction of a cent has nowhere to live in that column, and a price truncated toward zero is a cost breaker that never trips. No SMS vendor has been taken, so there is no contract to quote a figure from, and a number written into this repository without one would be invented. **The SMS branch refuses while it is unset.**
...
- **`MERIT_ECONOMIC_CALENDAR_HORIZON_TRADING_DAYS` is NAMED here and VALUED nowhere ([ADR-240](../decisions/ADR-240.md)).** It is the staleness horizon the Tier-1 economic calendar check compares against, and it is the **same number the panel reads**: [CRON_INVENTORY](../ops/runbooks/CRON_INVENTORY.md)'s Tier-1 row asserts that the newest `economic_calendar_loads.coverage_end_day` still runs ahead of today by the configured horizon, and `GET /economic-calendar` publishes `freshness.stale` against it. Set it on the **`api`** service and wherever the alarm runs, to the same value; two values would be a panel and an alarm disagreeing about the same calendar, which is exactly what one shared name exists to stop. It is a **threshold and not a credential**, so it is rowed here beside its alarm rather than in section 7's vault. **The unit is TRADING DAYS and the name says so**, because [ADR-146](../decisions/ADR-146.md) exists because two date vocabularies met and nothing said which was which. **Absent, `GET /economic-calendar` answers 503** rather than publishing a freshness verdict nobody decided, on [ADR-226](../decisions/ADR-226.md)'s rule that an absent configuration refuses rather than switching a control off. **No number is written into this repository** and [ADR-146](../decisions/ADR-146.md) clause 7 is why: no approved document states one, and a value in a route file would be a monitoring threshold on the wrong side of the standing parameter ruling.
