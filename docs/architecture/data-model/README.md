...
| [`reconciliation_runs`](reconciliation_runs.md) | `0064`. **The sweep, where [`reconciliations`](reconciliations.md) is one comparison.** Session 374's `B4`: `integrations.recon.last_run_at` had no source rather than a missing join, because every row of the neighbour above is about one account on one day and the only fold across them is the one [ADR-199](../../decisions/ADR-199.md) section 5 refuses for the batch. Mutable, and `reconciliation_runs_completed_is_whole` is why a sweep that stopped at the account boundary cannot claim it completed |
...
The tables below, created by [`0005`](../../../packages/db/migrations/0005_affiliate_program.sql), [`0012`](../../../packages/db/migrations/0012_disputes_and_affiliate_settlement.sql), [`0017`](../../../packages/db/migrations/0017_events_and_audit.sql), [`0018`](../../../packages/db/migrations/0018_integrations.sql), [`0019`](../../../packages/db/migrations/0019_notifications_and_community.sql), [`0020`](../../../packages/db/migrations/0020_public_surface.sql), [`0021`](../../../packages/db/migrations/0021_transparency.sql), [`0022`](../../../packages/db/migrations/0022_analytics_journal.sql), [`0023`](../../../packages/db/migrations/0023_loyalty_and_graduation.sql), [`0025`](../../../packages/db/migrations/0025_reserved_sequence.sql), [`0040`](../../../packages/db/migrations/0040_report_schedules.sql) and [`0073`](../../../packages/db/migrations/0073_operator_directory.sql). They are grouped below by the migration that creates them, which is also how they group by module.

**This sentence opened with a count and the count is deleted rather than corrected** ([ADR-034](../../decisions/ADR-034.md)'s remedy has exactly two branches, generate the value or delete it and point at the source). It read *"Thirty-seven tables"* against **thirty-eight** rows in the table below, measured on the commit that added [`operators`](operators.md) and [`operator_sessions`](operator_sessions.md) and wrong before either of them arrived. The list is the source.
...
| [`operators`](operators.md) | `0073`, [ADR-237](../../decisions/ADR-237.md). **Merit's own operators and their role over API_CONTRACT section 8's closed three.** It is the referent `admin_actions.actor` did not have: that column was `text NOT NULL` with no foreign key, so *"NO UNEXPLAINED ADMIN ACTION, EVER"* rested on a constraint any string satisfies. **A directory and deliberately not a login**: no password, no secret, no local credential |
| [`operator_sessions`](operator_sessions.md) | `0073`, [ADR-237](../../decisions/ADR-237.md). **What a verified assertion turns into, and nothing in this repository can write a row.** The minter needs the `C-08` identity provider, which is a purchase. The session lifetime carries a floor and **no ceiling**, which is stated as owed rather than invented |
| [`firm_parameters`](firm_parameters.md) | `0074`, [ADR-252](../../decisions/ADR-252.md), building [ADR-238](../../decisions/ADR-238.md) ruling 1. **The firm's own integer-valued numbers, and the first member is the base account cap.** `identities.max_accounts_override` is the per-entity exception and, before this table, nothing held the number it is an exception to. **The vocabulary is closed by a `CHECK` and closed again by the column type**, on `operators.role`'s precedent: a parameter that is not an integer has nowhere to go here. **Ships empty**: a cap is a launch candidate confirmed at launch as a row, and an absent row is NO CAP rather than an unlimited one |
...
| **Every entry resolves to one of the eight declared classes** | **LEDGER-C2** `ledger_entries_class_declared`, a `BEFORE INSERT` trigger, plus the `CHECK` on `ledger_accounts.code`. Both are stated in the LAST migration to install them ([`0056`](../../../packages/db/migrations/0056_eighth_ledger_code.sql)), never in `0009` or `0027`, which are superseded and never edited |
...
### The <!--gen:migration_files-->74<!--/gen--> files, and which of them are money path
