...
**It is a registered relation as of [ADR-209](../../decisions/ADR-209.md), and it is the only one in this tree that is not a table.** `TableKey` `economicCalendarCurrent`, scope class **`firm`**, which is `economic_calendar`'s class rather than a second judgement: a view declares no column, no foreign key, no constraint and no key of its own, so the relation it PROJECTS is the only one whose DDL can decide the tenancy. `firmDb().rows('economicCalendarCurrent')` is the door, and it is what stops the M04 panel's adapter reaching for the base table and re-deriving the maximum revision in application code.

**It has no key and must never be given one.** The `id` it projects is `economic_calendar`'s `PRIMARY KEY` and not the view's, so `packages/db/src/schema.ts` transcribes no key and every addressed write is refused before the database -- which is also what the database would say, because a `DISTINCT ON` view is not auto-updatable. `packages/db/test/scoped-db.test.ts` checks the transcription against this `CREATE VIEW`'s own SELECT list and against `economic_calendar`'s folded column types, rather than against a `CREATE TABLE` it does not have.

**It still has no design record of its own, and `CI-06i` is why.** That gate reconciles `CREATE TABLE` against `### <name>` records in both directions, so a file for a view would fail it as a record no migration creates. Whether the gate should reconcile views too is [ADR-209](../../decisions/ADR-209.md) section 8's open question.
