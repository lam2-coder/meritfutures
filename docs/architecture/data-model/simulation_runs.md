...
| `calibration_observed_at` | date | not null | **Unit: rail clock**, ruled by [ADR-082](../../decisions/ADR-082.md) and argued below rather than picked. The day the figures were **observed**, never the day of the run: `observedAt` is validated against `yyyy-mm-dd` in `provenance.ts`, so an observation day carries no time of day and no timezone and inventing either would be a figure Merit made up |
...
#### The unit token is argued, and [ADR-082](../../decisions/ADR-082.md) ruled it

**This section is kept and amended rather than deleted** ([ADR-065](../../decisions/ADR-065.md) T3). It was written as an open argument, because when this column landed the closed vocabulary had no word for it. **ADR-082 closed that**: the token is `rail clock`, it was correct on the day it was declared, and it is the third token's DEFINITION that moved rather than the token list.
...
**`rail clock` is declared, and it is now the token's stated meaning rather than only its operative half.** The definition reads *"a third party's own clock, quoted and never computed by Merit: a payment rail's, a calibration vendor's, any counterparty whose day Merit reads and never derives"*, and `rail` names the class after its commonest member. [ADR-042](../../decisions/ADR-042.md)'s structure is what carries the decision: two units Merit computes, and one category for a date that arrives from outside and is merely quoted. [`affiliate_commissions.chargeback_window_ends_on`](affiliate_commissions.md) carries the same token for the same reason, that the window "is the card networks' rather than ours".

**It used to read *"the rail's own clock"*, and that is why this argument exists at all.** Under that wording the predicate was exactly true here and the noun was not, so the token was declared with the mismatch stated in the open and `OI-28` was opened rather than the nearest-looking token being picked quietly. **Picking quietly is how three rows passed `CI-06m` by accident** before the marker was tightened, which is recorded in `gates.mjs` beside `UNIT_MARKER`.

**A fourth token was refused, and the reason binds the next date column that arrives from outside.** Three tokens partition on one question with exactly three answers: who answers this date, and may Merit compute it. Merit's exchange calendar, Merit's own clock, or neither. **A fourth would partition on who the counterparty is, which has no finite answer set**, and a KYC provider's date or a market-data feed's would each have as good a claim to a token of its own. So a date this table gains from any outside party is `rail clock`, and it needs no new argument. [ADR-082](../../decisions/ADR-082.md) sections 3 and 4 carry the full reasoning, and `CI-06m/widened-definition-is-not-a-fourth-token` in [`falsify.mjs`](../../../scripts/corpus/falsify.mjs) watches `vendor clock` being refused, so the closure is a control rather than a sentence.

**`OI-28`'s row in [DELTA_MANIFEST section 16](../../../packages/db/DELTA_MANIFEST.md) still reads `open`.** ADR-082 closes the item and that file was outside its fence, so the row is amended by a session that holds it. Stated here because a reader standing at this column is the one most likely to follow that link.
