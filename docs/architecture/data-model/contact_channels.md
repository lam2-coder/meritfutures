...
Constraints: `contact_channels_kind_allowed`; `contact_channels_supersession_is_complete`; `contact_channels_no_self_supersede`; **`contact_channels_ciphertext_is_complete`**; **`contact_channels_ciphertext_refuses_plaintext`**.

**`contact_channels_ciphertext_refuses_plaintext` is `INV-M10-12` as a constraint rather than as a promise.** `value_ciphertext` is `bytea` and every byte string is a valid `bytea`, so a handler that skipped the seal and wrote the address itself would satisfy every other constraint here, every index and every foreign key. Nothing would object, the row would read as sealed to every catalogue query, and the defect would surface at **decrypt** time with the address in the clear at rest in the meantime. The floor is `octet_length(value_ciphertext) >= 29` when the column is not null: a 12-byte nonce, a 16-byte GCM tag and at least one byte of ciphertext.

**A raw E.164 number is at most 16 bytes**, a `+` and at most fifteen digits, so every telephone number in existence sits below the floor. **The refusal is total for telephone numbers and partial here for the other two kinds**: `kind` is `('email','push','sms')`, so a plaintext email address of 29 bytes or more clears it. On [`identity_phones`](identity_phones.md) and [`phone_change_requests`](phone_change_requests.md) the column holds a telephone number and nothing else, so there it is total.
