...
~~**Nothing forces a `trading_calendar` update to write one of these rows.**~~ **`CALENDAR-C1` does, since [`0033`](../../../packages/db/migrations/0033_trading_calendar_revision_required.sql) under [ADR-045](../../decisions/ADR-045.md).** A `trading_calendar` `UPDATE` that commits with no row here carrying **that row's** `to_jsonb(OLD)` is refused, and so is one whose `dependent_row_count` is not the number the database itself counts across [`fills`](fills.md), [`daily_marks`](daily_marks.md) and [`rule_states`](rule_states.md). **The count is the reason the second half exists**: `trading_calendar_revisions_incident_named_when_dependent` reads the claimed number and nothing else, so before `0033` an incident became an ordinary data change by typing a zero. `OI-06 (calendar prior image)` is closed.

**`dependent_row_count` is therefore asserted twice and by two different things**, and they are not redundant. The `check` on this table says a non-zero count names an incident. The trigger says the count is true. **Either alone is a form**: a true count nobody must justify, or a justification attached to a number the writer chose.

**The image must be built by the database.** `prior_row = to_jsonb(OLD)` is a `jsonb` comparison, so an image assembled in application code does not equal it: a hand-written column list renders timestamps differently and is missing whatever a later migration adds. That is the enforcement of the derived-not-listed rule two rows above, rather than a restatement of it.
