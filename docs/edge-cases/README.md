...
**EC-150 to EC-156 are [FOLD-02](../plans/FOLD-02-enforcement-window-and-suspension.md)'s**, added by its registries session. **Four of the seven are one shape**, and the shape is the fold's whole finding: `EC-150`, `EC-151`, `EC-154` and `EC-156` each record an artifact that reads as a control and is not one, which is the same defect [EC-146](EC-146.md) recorded one fold earlier and this fold met four more times. `EC-156` is the sharpest, because there the entire vocabulary of a halt existed as columns and **nothing refused settlement**. **[EC-040](EC-040.md) was amended rather than added**: it stated `G-NO-IN-FLIGHT` with a value [ADR-028](../decisions/ADR-028.md) had retired and without the one that replaced it, and it is one of the four sites that ADR's own sweep named and did not reach.
...
**[EC-158](EC-158.md) is the first entry here whose module is `tooling` rather than a plan**, added by [ADR-300](../decisions/ADR-300.md). It records a defect in an untracked merge tool, so it owns no product behavior and declares `none owned` for its golden scenario. It is worth reading for one reason: **the only check that catches it has now fired on five consecutive merges**, and the tree measurement says that is why `main` is clean rather than evidence the check is noisy. A registry every other entry of which carries a product module is an imperfect home for it, and [ADR-300](../decisions/ADR-300.md) section 10 puts that to a founder read.

**[EC-157](EC-157.md) is the second of that kind and it is the sharper one**, added by the day-sequence generator session. EC-141 was an ambiguity that a reading resolved without amending either document. **EC-157 is a contradiction that no reading resolves**: `INV-18` and `INV-19` put the adjustment at the open and `daily_marks_balance_arithmetic` puts it inside the day, the three agree only where `adjustment_cents` is zero, and that is the value `SD-01` exists to make impossible to assume. It is `status: open` because both repairs are money path and the choice is the founder's. It was found by the first session that had to CHAIN a balance across days rather than read one, which is the only exercise that puts the two arithmetics on the same page.
...
| [EC-150](EC-150.md) | A clock in the schema with nothing scheduled to reach it  (2026-08-16, module: M05, status: specced) |
| [EC-151](EC-151.md) | A releaser that reports success is not evidence the release happened  (2026-08-16, module: M05, status: specced) |
| [EC-152](EC-152.md) | A second enforcement becomes a route around the first one's clock  (2026-08-16, module: M05, status: specced) |
| [EC-153](EC-153.md) | A reversible enforcement whose reversal depends on an artifact nobody has yet  (2026-08-16, module: M02, status: specced) |
| [EC-154](EC-154.md) | An outbound rail gated at one door and not the other, and the citation was inside an ADR  (2026-08-16, module: M08, status: specced) |
| [EC-155](EC-155.md) | A hard limit that binds one payment method and reads as binding all three  (2026-08-16, module: M03, status: specced) |
| [EC-156](EC-156.md) | A halt that is representable and unenforced  (2026-08-16, module: M20, status: specced) |
| [EC-157](EC-157.md) | The mark identities and the constraint that checks them place the adjustment on opposite sides of the open  (2026-08-16, module: M1 + M2, status: open) |
| [EC-158](EC-158.md) | An additive conflict resolution duplicates a generated line, and the only check that sees it looks like the noisy one  (2026-08-30, module: tooling, status: specced) |
