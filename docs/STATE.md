...
| **CI-03, golden files** | **DONE** (S-D, 2026-08-15) | [`packages/golden-loader`](../packages/golden-loader/README.md) over [`packages/rules-engine/fixtures`](../packages/rules-engine/fixtures/README.md), wired in [`.github/workflows/golden.yml`](../.github/workflows/golden.yml). Twelve loader rules, each watched failing on its own seeded violation. **Eighteen fixtures, not the registry's full set** ([session 45](sessions/2026-08-16-session-45.md)): the batch that the fixture format can state without inventing an input, every value traced to a plan document rather than to an engine. The rest wait on a calendar, a second plan record, or a payout evaluation that is not a day fold |
