...
1. **The $240 versus $300 per-day reconciliation on Merit Rapid.** [ADR-018](DECISIONS.md) records a recalibrated per-day ceiling of about **$240** (24,000c). The published parameters imply **30,000c**: a 100,000c cap at 50K, a 9000bp split, a 3 trading day cycle. Roughly 25 percent apart, in the direction where the model **understates** extraction. Neither number has been silently preferred; $240 is the number of record until `research/calibration/mc_lifecycle.py` is committed, and settling this is the first thing that file does.
2. **The reserve model has no cushion left.** [ADR-013](DECISIONS.md) recorded that the lifecycle simulation was basis anchored, which made realized liability *at most* the modeled figure under the settlement anchor. [ADR-019](DECISIONS.md) moved the anchor back to the basis day, so that margin is **spent**, not exceeded. CVaR99 and RE-S-01's bands are central estimates from here on. Any cushion wanted now has to be added deliberately and priced.
3. **Core EOD and Direct got faster and nobody asked them to.** ADR-019's anchor move compresses their cycles from 7 to 8 trading days to **5**, raising per-day extraction from 16,875 to 19,286c up to **27,000c**. This follows correctly from the wallet ruling and is within the (now cushionless) model, but it was a consequence of the wallet rather than a decision about Core EOD, and it is worth confirming that it was intended for those two plans and not only for Merit Rapid.
4. **The lineup no longer lands on one design ceiling.** The constitution's approximately $190 per day figure belonged to the settlement anchor. The three plans now sit between 27,000c and 30,000c per trading day. The `w=3` recalibration prices the higher level; the constitution's stated figure is simply superseded and should be read as such wherever it appears.
5. **`V-M2-15` is now commercial.** Fail-closed provisioning means the vendor relationship cannot support Merit's risk posture without an acknowledgement artifact or a readable risk setting. This changes what the vendor call is for.
6. **ADR-020 costs roughly 2 to 4 weeks** at Wave 4, the largest single scope addition since the constitution. Recorded as a duration so it can be traded against something if the schedule tightens.
7. Remaining commercial judgments, unchanged: OQ-M8-01 (new-affiliate reserve holdback), OQ-M8-02 (flat commission rate), OQ-M3-02 (refund window stated precisely enough to publish), OQ-M5-02 (freeze expiry, proposed 10 business days), OQ-M5-05 and OQ-M5-06 (top-up threshold, wallet-spend velocity), OQ-M6-02 (breaker minimum sample), OQ-M7-03 (severity-5 SLA).
...
3. **Commit `research/calibration/mc_lifecycle.py` and the business-model workbook**, then settle item 1 under "needs founder eyes". This is a founder action and it unblocks the harness.
