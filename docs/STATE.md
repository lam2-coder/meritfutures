...
| ~~**M01 disagrees with itself on R-15's lock effect**~~ **RULED, PENDING SIGNATURE: [ADR-052](decisions/ADR-052.md) (2026-08-17)** | Section 3.4's `max` against section 3.6's assignment, above. **The ADR rules for the ASSIGNMENT, against the engine as built**, on `CV-11`'s derivation of `INV-21`: the `max` makes "post-lock it equals it" false, a three-payout drain then puts a funded account one cent below its own floor with no losing day, and **no publish-time check can bound a runtime quantity**. The `INV-06` objection this row was recorded under is a **sub-step artifact**: session 45 tightened 3.6's tripwire to fire between sub-steps, which is stricter than any stated invariant, and that tightening is what forced the `max`. **Status `proposed`; the signature is the founder's** |
