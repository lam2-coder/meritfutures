...
**<!--gen:adr_count-->43<!--/gen--> ADRs. <!--gen:ec_count-->141<!--/gen--> edge cases. <!--gen:gs_count-->257<!--/gen--> golden scenarios. Four waves.** These are generated spans under [CI-06g](testing/STRATEGY.md); this line read "25 ADRs" until it was folded, which is the drift [ADR-034](decisions/ADR-034.md) exists to end.
...
**The wall-clock audit the plan was commissioned to run came back clean, and it is a measurement rather than a reassurance.** Across all <!--gen:migration_files-->30<!--/gen--> merged migrations: **zero `::date`, zero `CAST(`, zero `interval` in any case, no `date` column with any default, and every `now()` a `DEFAULT now()` on a `timestamptz`.** **No existing trading-day counter is computed against a wall clock.**
...
## FOLD-02 session 3 landed: `0030` and `0031` (2026-08-16)

**[ADR-040](decisions/ADR-040.md) and [ADR-041](decisions/ADR-041.md) are in the database.** Two files, because one is not atomic. [`0030`](../packages/db/migrations/0030_payout_hold_enum.sql) is `ALTER TYPE payout_status ADD VALUE 'held_pending_review'` and nothing else, with no transaction block. [`0031`](../packages/db/migrations/0031_payout_hold_and_identity_restriction.sql) carries the hold trio and its completeness CHECK, **both `SD-09` predicates** dropped and re-created under their existing names, the hold-expiry index, `wallet_withdrawals_frozen_cannot_settle`, `identity_restriction_episodes`, and the replacement `COMMENT ON TABLE payout_requests`. **Both need the founder's E2 read and neither is merged.**

| | |
|---|---|
| **The counterfactual, and its own counterfactual** | The combined form was watched failing (`unsafe use of new value`). Then the **same statements with the transaction block removed** were watched **succeeding**. So the split's reason is that **`0031` must be atomic**, not that the statements cannot share a file, and FOLD-02 finding 6's wording is corrected in [DELTA_MANIFEST section 14](../packages/db/DELTA_MANIFEST.md). Both directions are a CI step that **fails the build if the combined form ever succeeds** |
| **Executed, not read** | All <!--gen:migration_files-->30<!--/gen--> migrations apply forward-only from empty against PostgreSQL 16: <!--gen:sql_tables-->97<!--/gen--> tables, 331 indexes, 353 check constraints, 6 triggers, from the catalogue rather than a grep. [`probe_payout_hold.sql`](../scripts/db/probe_payout_hold.sql) is **18 / 18**, leading with four success cases |
| **`CI-06g` could not see its own `e2_files` spans** | `spansIn()` matched span names against `[a-z_]+` and `e2_files` is the one key with a digit in it. Four spans, never compared, never regenerated, always green. They read **18**, true when written and stale from the moment `0028` merged, and the figure they carry is **the scope of the E2 read**. The tenth site of the hand-maintained-count class, and the first inside the gate written to end it |
| **Two departures from the approved plan** | `wallet_withdrawals_open_idx` is **not** re-created, because its predicate already matches halted rows; and `payout_requests_hold_is_complete` deliberately **keeps** its trio after release, where the merged freeze constraint clears it. Both are founder calls at the E2 read and both are recorded rather than absorbed |
| **What is not implied** | **`identities.status = 'restricted'` still blocks nothing.** `G-ELIGIBLE` gaining the identity status is session 4's, with the machines. The schema is ready; nothing enforces it yet |

---

## Next 3 actions

1. **The founder's E2 read** on the <!--gen:e2_files-->20<!--/gen--> money-path migration files, and a ruling on item **B** ([ADR-030](decisions/ADR-030.md)'s stale config list, wrong in two of four). **A** and **C** are closed. Nothing merges first.
...
|---|---|---|
| **The reconciled schema and migrations** | **DONE**, pending the E2 read | <!--gen:migration_files-->30<!--/gen--> files, 96 tables, 326 indexes, 347 check constraints, 6 triggers, verified on a clean PostgreSQL 16 install. Nothing to build. **The founder's read is the remaining work and it is not engineering** |
...
| **CI-06h, migration install** | **RUNS IN ACTIONS, green** | Corrected 2026-08-15 (S-B). This row read "WIRED, never executed by GitHub. **It has not run in Actions once**" and that was already false when it was written: run `31860712550`, job `94953489824`, commit `3082b61e`, **2026-08-15T03:01:16Z, success**, applying all <!--gen:migration_files-->30<!--/gen--> migrations against PostgreSQL 16 on a runner. It has since run on every push to this branch, and now carries ADR-035's probe as well (run `31862563569`) |
...
| **TradingCalendar as data** | **PLANNED** (S-E, 2026-08-15). Schema only on disk | `trading_calendar` exists in `0004` with its ruled semantics (half day counts as a full day, a halt advances counters but not win days). **There is still not one row of data anywhere in the repository, and no seed mechanism**: `grep -c 'INSERT INTO' packages/db/migrations/*.sql` is zero across the set. [P1-SE-trading-calendar](plans/P1-SE-trading-calendar.md) is `approved` and is five build sessions, four of them money path |
