...
|---|---|---|
| **CI-06, corpus integrity** | **DONE and exceeded** | <!--gen:gate_count-->18<!--/gen--> checks, all passing clean and failing dirty. **That figure was the bare word "Twelve" until 2026-08-17**, which is a hand-maintained count in the row that reports the gate against hand-maintained counts; it is a span now, on ADR-034's remedy, and the query is `GATES.length`. The row's own definition of done is met **for CI-06 only**. This row stood three times in this table and read "Eleven" against the twelve `gates.mjs` reports; both are `OI-10` |
