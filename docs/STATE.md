...
**R-14's tripwire did not fire, and that is the more useful half of the finding.** It compared the final floor against the floor at the **start of the day**, so a retreat introduced *between* the trail and the lock stayed inside the interval it sampled: 5,010,000 is still above the 4,750,000 it opened at. **A monotonicity check that only samples the endpoints cannot see a retreat inside the interval.** It now runs after each sub-step.
...
|---|---|---|
| **CI-06, corpus integrity** | **DONE and exceeded** | <!--gen:gate_count-->17<!--/gen--> checks, all passing clean and failing dirty. **That figure was the bare word "Twelve" until 2026-08-17**, which is a hand-maintained count in the row that reports the gate against hand-maintained counts; it is a span now, on ADR-034's remedy, and the query is `GATES.length`. The row's own definition of done is met **for CI-06 only**. This row stood three times in this table and read "Eleven" against the twelve `gates.mjs` reports; both are `OI-10` |
...
| **The reconciled schema and migrations** | **DONE**, pending the E2 read | <!--gen:migration_files-->37<!--/gen--> files, **98 tables, 332 indexes, 359 check constraints, 6 triggers**, verified on a clean PostgreSQL 16 install (`0001` to `0028` then `0032`; `0029` to `0031` are reserved and unwritten). The figures read 96 / 326 / 347 / 6 before `0032`. Nothing to build. **The founder's read is the remaining work and it is not engineering** |
...
**Position at planning time, each figure produced by a command rather than quoted:** `6b7a41c`, 17 of 17 gates, 7 of 7 repo invariants, 54 test files with 789 passing, <!--gen:migration_files-->37<!--/gen--> migrations, 46 of 50 rules declared, 36 of <!--gen:gs_count-->284<!--/gen--> fixtures, and **one of P2's seven properties asserted**, which is what three of the wave's ten sessions exist to move.
