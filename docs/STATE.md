...
| **The proof** | [`probe_reversible_contact_addresses.sql`](../scripts/db/probe_reversible_contact_addresses.sql), **27 assertions, 27 / 27**, eleven of them successes, wired into `CI-06h`'s job. **Eight seeded schemas, each watched failing on its own finding** |

**The seeded run corrected the file, which is the transferable part.** The trailing `REVOKE DELETE ... FROM merit_dispatcher` carried a comment calling it "a statement rather than a mechanism". The seed added `DELETE` to the grant list above it and **all 27 assertions still passed**, because the `REVOKE` had already taken it back: a privilege granted earlier in the file cannot survive to `COMMIT`. **That is the third comment in three days found wrong in the direction of understating a control, and the second found by execution rather than by reading.** The argument it makes is not "write more careful comments"; it is that a seeded violation is worth running even when you are confident what it will say.
