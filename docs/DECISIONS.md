...
|---|---|---|
| 001 to 025 | merged to `main` | **allocated** |
| **026 to 032** | `claude/schema-delta-reconciliation` (PR #4) | **reserved, unmerged.** ADR-026 to ADR-030 are the schema-delta fold; ADR-031 and ADR-032 are the `published_statistics` rulings. **These numbers are load-bearing**: they are cited in migration comments, `DELTA_MANIFEST` and `DATA_MODEL`, and **a migration comment becomes sacred at merge** |
| **033** | `claude/builder-reviewer-loop-rykvhs` (PR #5) | **reserved, renumber pending.** That branch currently carries the citation-reviewer ADR as `ADR-031`, which collides with PR #4. **Founder ruling: PR #4 keeps 031 and 032; PR #5 moves to 033**, because PR #4's numbers are embedded in files that stop being editable when they merge and PR #5's is cited only within its own branch. **The renumber is applied on that branch, not here** |
