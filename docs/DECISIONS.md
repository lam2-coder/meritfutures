...
| OQ-7 | Identity-level extraction ceiling | **No identity ceiling in v1.** AS-09 is handled by visibility (the identity-level Eligible-Next-7-Days forecast plus [ADR-011](#adr-011-reserve-funding-is-weekly-manual-with-a-same-day-top-up-trigger--2026-08-13-status-accepted)'s same-day top-up trigger), not by a rule. The engine stays per-account and pure, and no cross-account state enters the fold. M6 owns the forecast; M7 owns the correlation detector |
...
|---|---|---|
| Reserve `promotional_credit` ledger class and the `currency` columns on `ledger_entries` and `purchases` now | [DATA_MODEL §16.5](architecture/DATA_MODEL.md#16-founder-rulings-wave-2-gate-2026-08-13) | **Confirmed.** Both stay reserved and unused in v1 math, with `currency` defaulting to `USD`. The cost is two columns and one row; the migration avoided is a multi-currency retrofit onto a live ledger |
...
| Payout endpoint takes an optional amount, defaulting to maximum eligible | [API_CONTRACT §13.2](architecture/API_CONTRACT.md#13-founder-rulings-wave-2-gate-2026-08-13) | **Confirmed with an amendment.** See [ADR-009](#adr-009-payout-amount-is-optional-and-defaults-to-the-maximum-eligible--2026-08-13-status-accepted) |
...
| Dual control on cap, split, and gap edits | [API_CONTRACT §13.4](architecture/API_CONTRACT.md#13-founder-rulings-wave-2-gate-2026-08-13) | **Confirmed with a launch-scale note.** See [ADR-010](#adr-010-dual-control-on-cap-split-gap-and-treasury-credentials-with-both-keys-founder-held-at-launch--2026-08-13-status-accepted) |
...
| Admin origin | [INFRA §13.2](architecture/INFRA.md#13-founder-rulings-wave-2-gate-2026-08-13-and-remaining-questions) | **Separate apex domain, placeholder `ADMIN_ORIGIN`.** See [ADR-012](#adr-012-admin-console-lives-on-a-separate-apex-domain--2026-08-13-status-accepted) |
| Reserve funding cadence | [OVERVIEW §10.3](architecture/OVERVIEW.md#10-founder-rulings-wave-2-gate-2026-08-13) | **Weekly manual plus a same-day top-up trigger.** See [ADR-011](#adr-011-reserve-funding-is-weekly-manual-with-a-same-day-top-up-trigger--2026-08-13-status-accepted) |
...
|---|---|---|---|
| 1 | **A reversed ruling.** [ADR-027](DECISIONS.md) was first written ruling **one** per-identity class named `trader_wallet`, retiring `trader_withdrawable`. It was folded into GLOSSARY, DATA_MODEL and M05, and **committed** | Collapsed two positions that move by **different magnitudes in the same transaction** (`approved_cents` against `trader_cents`) into one | **Re-reading M05's own text**, which stated the split explicitly and had been read past |
