...
import { AVAILABILITY, REQUIRED_ENDPOINTS, SIGN_IN } from '../src/app/sign-in/availability.ts';
...
describe('the four routes behind SC-M4-01', () => {
  test('every one is spelled as API_CONTRACT spells it', () => {
    ...
    for (const factor of SIGN_IN.factors)
      expect(REQUIRED_ENDPOINTS as readonly string[]).toContain(factor.endpoint);
  });
});
