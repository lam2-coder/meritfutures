...
// WOULD BE A SECOND COPY OF IT. `apps/api/src/routes/auth.ts:674` sets
...
// API_CONTRACT line 19 and in `auth.ts:674`, and nowhere in `SECURITY.md`. The
...
// `auth.ts:1178` and `auth.ts:1233` establish a session and `auth.ts:1243`
...
// registered routes use it (`auth.ts:1132` and `auth.ts:1208`), and
