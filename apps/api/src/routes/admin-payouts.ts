...
export function adminPayoutHandler(spec: AdminPayoutEndpointSpec): RouteHandler {
  return async (request: FastifyRequest, reply: FastifyReply): Promise<unknown> => {
    ...
    try {
      ...
      // in the log, where `AdminPayoutUnwired`'s message names the port member.
      let principal: AdminPrincipal | null;
      try {
        principal = await active.principal(request);
      } catch (err) {
        if (!(err instanceof AdminPayoutUnwired)) throw err;
        request.log.error({ err }, 'admin payout backend is not wired: principal');
        return sendProblem(
          reply,
