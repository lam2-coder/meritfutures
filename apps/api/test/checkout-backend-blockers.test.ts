...
test('ADR-238 ruling 1: one migration line names an account cap and it is the per-entity exception', () => {
  // This is the assertion `RI-20` runs against the BLOCKED reason's quoted
  // grep, held here as a case as well so that it fails inside the suite rather
  // than only inside the invariant runner.
  const lines = migrationLinesMatching('max_accounts');
...
test('ADR-238 ruling 1: no migration declares the BASE cap under the name the corpus gives it', () => {
  ...
  // jsonb and no column materializes it, which is why ruling 1 has somewhere to
  // stand: the value the corpus states exists, and the enforcement the
  // constitution requires cannot read it from there.
  expect(migrationLinesMatching('max_accounts_per_entity')).toHaveLength(0);
});
 ...
 * by the suite.
 */
function migrationLinesMatching(needle: string): readonly string[] {
  const dir = join(ROOT, 'packages', 'db', 'migrations');
  const names = readdirSyncSorted(dir);
  const hits: string[] = [];
  for (const name of names) {
    const lines = readFileSync(join(dir, name), 'utf8').split('\n');
    for (const line of lines) if (line.includes(needle)) hits.push(`${name}: ${line.trim()}`);
  }
  return hits;
}
