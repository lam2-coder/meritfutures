...
test('the wired count is reported, so a regression is a number and not a paragraph', () => {
  ...
  expect({
    ...
    // 6 to 7 and 15 back to 14. `useWithdrawalBackend` was entered in `BLOCKED`
    // above on the ground that no `IdempotencyStore` implementation exists in
    // this tree, and THAT IS FALSE: `src/idempotency-store.ts:144` exports
    // `databaseIdempotencyStore`, covered by `idempotency-store.test.ts`, and
    // the `usePayoutBackend` entry a hundred lines up says so in its own words.
    // `WithdrawalBackend` is `transact`, `idempotency` and `now`, all three
    // have doors, so `start.ts` installs it and no entry here can be true.
    // BLOCKED MOVING BACK RATHER THAN UP is the difference between a door
    // landing and an excuse being added, which is what this number is for.
  }).toStrictEqual({ declared: 21, wired: 7, blocked: 14 });
});
