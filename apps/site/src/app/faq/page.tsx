// =============================================================================
// apps/site/src/app/faq/page.tsx
// =============================================================================
// `/faq`. ONE OF THE TWO CONTENT INDEXES WITH NO PAGE MODEL OF ITS OWN, AND
...
export default async function FaqSurface(): Promise<ReactElement> {
  ...
  if (build.kind !== 'wired') {
    return (
      <>
        ...
        <Unavailable surface="/faq" reason={build.reason} />
      </>
    );
  }

  try {
    const documents = await build.ports.content.listAll('faq', 'en');
    ...
    return (
      <Surface envelope={envelope}>
        {live.length === 0 ? (
          <p data-testid="no-faq-documents">This build read no published question.</p>
        ) : (
          live.map((document) => (
            <section key={document.slug} data-testid="faq-entry">
    ...
    console.error('the published questions could not be read', cause);
    return (
      <>
        <h1>Frequently asked questions</h1>
        <Unavailable
          surface="/faq"
          reason="The published questions could not be read for this build."
        />
      </>
    );
  }
