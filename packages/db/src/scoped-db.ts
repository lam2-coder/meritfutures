...
export function systemTx(
  ...
  return {
     ...
     * all.
     *
     * **THE RUNTIME CHECKS DO NOT COME OFF ON THE STRENGTH OF THE TYPE**, which is limit 1 of
     * `CatalogRow` (`scoped-db.ts:3466`), stated ONCE there and not restated here. The shape is
     * derived from a TRANSCRIPTION of the DDL. This read "nothing in this tree compares a
     * transcribed column type against the migration." `RI-14` (ADR-444): FALSE when written. A
     * caller reading money off one of these rows still checks the value it read.
     *
 ...
 * HERE.** The type is derived from a TRANSCRIPTION. This read "ADR-112 foreclosure 4 records that
 * no check in this tree compares a `schema.ts` column type against the DDL." `RI-14` (ADR-440):
 * FALSE when written, and foreclosure 4 is about EXHAUSTIVENESS. Its replacement RESTATED the
 * suite and closed "DEFAULT compared nowhere": `RI-14` (ADR-444), FALSE when written,
 * `scoped-db.test.ts:3217` compares it. **ADR-444 RULES THE RESTATEMENT OUT**: stated ONCE at
 * limit 1 of `CatalogRow` (`scoped-db.ts:3466`), cited here. A caller reading a value off one of
 * these rows STILL CHECKS IT; the type retires only the EXISTENCE guard, `ADR-299` section 5.1
...
/**
 * Refuse a values object that names a GENERATED column, STORED or ALWAYS IDENTITY.
 *
...
function refuseGeneratedColumn(key: TableKey, values: WriteValues): WriteValues {
  ...
  for (const named of Object.keys(values)) {
    for (const [property, column] of Object.entries(columns)) {
      ...
      if (column.generated === undefined && column.generatedIdentity?.type !== 'always') continue;
      // THE IDENTITY HALF IS GATED ON `always` AND NOT ON THE PROPERTY'S MERE
      // PRESENCE. `GeneratedIdentityConfig.type` is `'always' | 'byDefault'`, and
      // PostgreSQL ACCEPTS a write naming a `GENERATED BY DEFAULT AS IDENTITY`
      // column: refusing on `generatedIdentity !== undefined` would refuse a
      // statement the database allows, which is the mirror of the error `ADR-448`
      // section 8 item 1 made and `ADR-451` discharged. All 20 identity columns on
      // the registry are `always` today, so the two spellings are indistinguishable
      // on this estate and differ only on the column nobody has added yet.
      if (column.generated === undefined) {
        throw new Error(
          `"${named}" is ${key}.${column.name}, which schema.ts declares ` +
            '`GENERATED ALWAYS AS IDENTITY`, and a write never takes it from the caller. ' +
            'PostgreSQL refuses a statement that names it with 428C9. The database owns the ' +
            'value, so a caller supplying one would be overwriting the identity its own row ' +
            'is addressed by.',
        );
      }
      throw new Error(
