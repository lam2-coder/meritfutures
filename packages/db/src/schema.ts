...
// =============================================================================
// SIXTY-EIGHT TABLES OF 111, AND THAT IS REPORTED RATHER THAN ROUNDED UP. The
// other 43 are not reachable through either accessor: `SCOPE_RULES` is total
...
//
// THE SIXTY-EIGHT ARE NOT ONE PHASE'S SET AND WILL NEVER BE. ADR-092 makes the
...
// of the sixty-eight below carry later columns -- `sessions`, `plan_versions`,
...
// one-member vocabulary. It carries no `.references()` even though
// `payout_requests` IS now one of this file's tables: the rule above has two
// conjuncts and the first one fails, because the FK arrived by `ALTER TABLE ...
// ADD COLUMN` rather than inline in the `CREATE TABLE` body. The fold reads
// `ADD COLUMN` and deliberately ignores `ADD CONSTRAINT`, so a constraint
// claimed here would be a claim nothing in this package checks.
