...
**It has been used FIVE times more and no use took a row above, which is stated rather than repaired, and the sentence that stood here was AMENDED IN PLACE ON 2026-09-05 BY [ADR-337](../../docs/decisions/ADR-337.md) BECAUSE IT WAS FALSE** ([ADR-065](../../docs/decisions/ADR-065.md) T3). It read *"It has been used twice more ... `4b` records FOLD-03's single delta and `4c` records FOLD-04's"*. **Measured off the headings rather than reconstructed: `4b` heads one section, FOLD-03's [ADR-066](../../docs/decisions/ADR-066.md) delta, and `4c` heads FOUR** -- FOLD-05's [ADR-071](../../docs/decisions/ADR-071.md), FOLD-03's [ADR-067](../../docs/decisions/ADR-067.md), and FOLD-04's [ADR-068](../../docs/decisions/ADR-068.md) and [ADR-069](../../docs/decisions/ADR-069.md). **SO THE LETTERED SEQUENCE HAS COLLIDED FOUR WAYS AND NOBODY HAD COUNTED IT.** It is NOT renumbered here, on this table's own ruling for `OI-06` and for section `14`: renumbering breaks every citation of whichever one moves. **AND `RI-37` LEG 4 DOES NOT REPORT IT**, because a lettered heading takes no number and the leg compares numbers; the letters are excluded by the written rule in the sentence after this one, which is a scope and not an exemption. A gate over the lettered sequence is a row nobody has written and is named as owed rather than taken here. **A lettered section deliberately claims no number**, so the sequence this table allocates is undisturbed by either, and adding rows for them would make the table's own key ambiguous between a number and a name. **The sentence above is the allocation**: a fold that adds a delta section appends the next letter after `4c` and does not enter it here.
