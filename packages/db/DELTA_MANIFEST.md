...
- **`swept_value_bp` ships under a name that is not always true.** **It claims no `OI` number of its own** and is carried to the founder's `E2` read, recorded in the design record and in the migration header: [M21](../../docs/plans/M21-plan-designer.md) section 3.4's own worked example sweeps `max_payouts`, a count of 5 and not a basis point. The plan's row is the authority this migration transcribes, so the column keeps the plan's name and the mismatch goes to the founder's read.
