...
describe('the registry is total', () => {
  ...
  // rounded up: the other 24 are unreachable through either accessor.
  //
  // `identity_links` IS ONE OF THE 24 AND ITS ABSENCE IS DELIBERATE. It carries
  ...
  // `events` IS ANOTHER OF THE 24 AND ITS ABSENCE IS ALSO DELIBERATE. It reaches
  ...
  test('87 declared tables, 87 scope rules, 0 reachable without one', () => {
    ...
    expect(declared).toBe(87);
    expect(rules).toBe(87);
