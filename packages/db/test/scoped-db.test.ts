...
describe('the registry is total', () => {
  ...
  // rounded up: the other 26 are unreachable through either accessor.
  //
  // `identity_links` IS ONE OF THE 26 AND ITS ABSENCE IS DELIBERATE. It carries
  ...
  // `events` IS ANOTHER OF THE 26 AND ITS ABSENCE IS ALSO DELIBERATE. It reaches
  ...
  test('85 declared tables, 85 scope rules, 0 reachable without one', () => {
    ...
    expect(declared).toBe(85);
    expect(rules).toBe(85);
