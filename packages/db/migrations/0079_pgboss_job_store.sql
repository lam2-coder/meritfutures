-- =============================================================================
-- 0079_pgboss_job_store
-- =============================================================================
-- E2 READ: MONEY BY WHAT IT GOVERNS AND NOT BY WHAT IT TOUCHES. Not one cent
-- moves through any object below and no ledger column changes. What lands is a
-- schema INSIDE THE PITR BOUNDARY THAT PROTECTS THE LEDGER, in the same database
-- and inside the same restore, which is ADR-165's precedent and is the reason
-- `packages/queue/src/pg-boss-queue.ts` refuses a boot-time install in its own
-- words: "A queue that installs its own schema at boot puts tables inside the
-- PITR boundary that protects the ledger with no review and no migration
-- number."
--
-- ADR-318 (status: proposed, founder approval PENDING) is the ruling, on ADR-305
-- section 7 slice 4 and ADR-006. NOTHING HERE IS SIGNED.
--
-- -----------------------------------------------------------------------------
-- THE BODY IS A TRANSCRIPT AND NOT A DECLARATION
-- -----------------------------------------------------------------------------
-- Everything between the two VERBATIM markers below is the exact string
-- `getConstructionPlans(QUEUE_SCHEMA)` returned from the installed pg-boss. It
-- was not transcribed from pg-boss's documentation, its changelog or its git
-- history, and it was not typed. Byte for byte, including its indentation, its
-- blank lines, its doubled statement terminators and its own inline comments.
--
--   node scripts/db/assert_pgboss_schema_matches_library.mjs --emit
--
-- regenerates it, and the same script with no argument ASSERTS that this file
-- still quotes what the installed library emits. That check is the whole reason
-- the markers exist, and it runs in CI-06h.
--
-- EMITTED-BY: pg-boss@12.28.0 getConstructionPlans('pgboss'), schema version 38
--
-- `pgboss` is read from `QUEUE_SCHEMA`, declared once at
-- `packages/queue/src/pg-boss-queue.ts:66` "so the migration that installs it and
-- the runtime that reads it cannot disagree by a spelling". This file does not
-- spell it a second time: it quotes output that was generated FROM that
-- constant, and the assertion script re-reads the constant on every run.
--
-- -----------------------------------------------------------------------------
-- THIS MIGRATION IS CORRECT AGAINST ONE LIBRARY VERSION, WHICH IS WHY THE
-- VERSION IS IN THE HEADER RATHER THAN IN A COMMIT MESSAGE
-- -----------------------------------------------------------------------------
-- The last statement of the quoted body writes `38` into `pgboss.version`, and
-- that number is pg-boss's own schema version, taken from its manifest key
-- `pgboss.schema`. Under `migrate: false` the library runs `Contractor.check()`
-- at `start()` rather than `Contractor.start()`, and `check()` throws
-- `pg-boss database requires migrations` unless that row equals the version
-- compiled into the library it is running. SO A CATALOG BUMP OF `pg-boss` IS A
-- SCHEMA CHANGE, whether or not the person taking it thinks of it as one.
--
-- Migrations are sacred: once merged, never edited, only superseded
-- (constitution E2). The remedy for an upgrade is therefore always a NEW
-- numbered migration carrying `getMigrationPlans(schema, version)`'s output at
-- the same standard of transcription, never an edit to this file.
--
-- -----------------------------------------------------------------------------
-- `migrate: false` STAYS FALSE, AND THE LOUD FAILURE IS THE FEATURE
-- -----------------------------------------------------------------------------
-- `pg-boss-queue.ts:90` sets it and this file does not change it. Against a
-- database carrying this migration, `start()` succeeds. Against one without it,
-- `start()` throws `pg-boss is not installed` and the deployable does not come
-- up, which the queue's header calls "the right failure: the alternative is a
-- silent install".
--
-- -----------------------------------------------------------------------------
-- WHAT THIS FILE DELIBERATELY DOES NOT DO, EACH ONE A THING A READER WILL LOOK
-- FOR AND NOT FIND
-- -----------------------------------------------------------------------------
-- 1. IT GRANTS NOTHING, AND `merit_app` THEREFORE CANNOT USE THIS SCHEMA.
--    `0026_roles_and_grants.sql` grants USAGE, table privileges and default
--    privileges IN SCHEMA public and nowhere else, so after this file
--    `has_schema_privilege('merit_app', 'pgboss', 'USAGE')` is FALSE. That is
--    measured, asserted by `scripts/db/probe_pgboss_job_store.sql`, and left
--    that way ON PURPOSE. pg-boss's `create_queue` runs `CREATE TABLE
--    pgboss.%I`, so making the queue usable by the application role means
--    granting CREATE on a schema inside the ledger's restore boundary to the
--    role `0026` explicitly REVOKES CREATE from on public. That is a privilege
--    ruling on the money database, it belongs to the slice that wires a
--    deployable to the queue (ADR-305 slice 8), and inventing it here would be
--    this file deciding a control it was not dispatched to decide. `0074` and
--    `0076` refused an invented value on the same ground.
-- 2. IT WIRES NO DEPLOYABLE. No manifest gains `@merit/queue`, no job is
--    registered and nothing is scheduled. A job store installed and unused is a
--    known defect (ADR-239) and it is slice 8's to close.
-- 3. IT CREATES NO QUEUE AND ENQUEUES NOTHING. `pgboss.queue` ships empty, the
--    way `0074` shipped `firm_parameters` empty: a queue is a name the
--    application chooses, not a row a migration invents.
--
-- -----------------------------------------------------------------------------
-- TWO PROPERTIES OF THE QUOTED BODY WORTH KNOWING BEFORE READING IT
-- -----------------------------------------------------------------------------
-- A. IT IS NOT DETERMINISTIC IN THE OBJECTS IT CREATES. The `DO` block over
--    `pgboss.queue_stats` creates one partition per day for today and tomorrow,
--    named `queue_stats_YYYYMMDD` off `now() AT TIME ZONE 'UTC'`. Applying this
--    file on two different days installs two differently named tables. So a
--    table count over the `pgboss` schema is a fact about the day it was read,
--    and no gate in this repository states one.
-- B. IT OPENS AND CLOSES ITS OWN TRANSACTION and takes an advisory lock keyed on
--    the database and schema name. Nothing wraps it here, because wrapping it
--    would be editing the transcript.
--
-- Nothing in the body touches the `public` schema, so the object counts CI-06h
-- derives there are UNCHANGED by this file: 118 tables, 412 indexes, 516 checks,
-- 32 triggers, before and after, read from the catalog on PostgreSQL 16.13.
-- =============================================================================

-- >>> BEGIN VERBATIM pg-boss getConstructionPlans OUTPUT >>>
...
-- <<< END VERBATIM pg-boss getConstructionPlans OUTPUT <<<
