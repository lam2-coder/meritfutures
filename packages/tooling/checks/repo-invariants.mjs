...
// -----------------------------------------------------------------------------
// RI-26  Every statement of the last-closed-trading-day fold is registered, and
  ...
  run(root) {
    ...
    for (const file of sources) {
      ...
      if (names && entry === undefined) {
        findings.push(
          `${file} names a session-close instant in CODE and is not on RI-26's register. ` +
      ...
      if (!names && entry !== undefined) {
        findings.push(
          `${file} is on RI-26's register as "${entry.what}" and its code no longer names a ` +
