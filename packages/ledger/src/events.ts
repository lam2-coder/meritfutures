...
// -- and `apps/api` HOLDS NO TRANSACTION THAT CAN CARRY AN EVENT stands
// on all five rather than on two. **THAT SENTENCE SAID "THIS DEPLOYABLE" UNTIL
// ADR-410 MOVED THIS FILE OUT OF ONE**, and the deployable it is about is named
// outright now rather than left to a pronoun this file can no longer supply. That is the finding this slice reports and it
...
// deployable apart, and that was TRUE OF THIS FILE UNTIL ADR-410 AND IS KEPT
// HERE BESIDE ITS CORRECTION (`RI-14`). It read: "`apps/worker` holds every
// `emit` call site in the workspace and the ONE handle that can carry the write,
// and it cannot reach this producer in either spelling -- an `@merit/api`
...
// AND IS NOT THIS SESSION'S TO TAKE**."
//
// **THAT THIRD ADDRESS IS THIS FILE'S OWN AND THE MOVE IS TAKEN.** ADR-104 ruled
// exactly this shape for `packages/ledger` when two deployables had to post and
// `RI-04` forbade an app depending on an app, and ADR-410 re-derived that
// `@merit/db`, `@merit/ledger` and `@merit/rules-engine` are each already a
// dependency of BOTH deployables, so the move needed no new package, no manifest
// edge and no `VG-12` admission. `apps/worker` can NAME this producer from that
// commit. **IT IS STILL NOT WIRED TO IT**, deliberately: installing the sink is
// a slice with its own verification and ADR-410 declined to take it.
//
...
// -----------------------------------------------------------------------------
// THE CONCLUSION IS UNCHANGED BY THE MOVE AND THE REASON IS REPLACED BY A
// STRONGER ONE. It read: "`db.ts` is THE ONE FILE IN THIS DEPLOYABLE THAT NAMES
// `@merit/db`, by ADR-120, and a `grep -rln '@merit/db' apps/api/src` that
// returns one path is the whole of that convention." That was a convention about
// one deployable and this file is no longer in it. What holds now is a
// PROHIBITION: `packages/ledger` declares no dependency at all, cannot import
// `client()`, and cannot import `drizzle-orm` or `pg` because `VG-4` bans both
// outside `packages/db`, which `tx.ts` states of this package one file over. So
// this file imports NOTHING, declares its shapes structurally, and takes the
// write as a port -- which is `apps/worker/src/sweeps/ports.ts`'s disposition
// and `LedgerTx`'s beside it, all three arrived at independently and for a
// compatible reason.
//
 ...
 *
 * THE TRANSACTION IS `object` AND THAT IS NOT LAZINESS. `apps/api`'s handles
 * come from `@merit/db` and the worker's come from
 * `apps/worker/src/sweeps/ports.ts`, and this package may name neither: it
 * declares no dependency and `RI-04` puts the second out of reach of everything.
 * A producer that does not read the handle has no business narrowing it, and the
 * widest supertype is what lets ONE producer serve both without either one
 * casting. That was ADR-120's argument from inside `apps/api` and it is this
 * package's own prohibition now, which is a stronger footing for the same shape.
 */
...
// -----------------------------------------------------------------------------
// THIS BLOCK SAT BELOW `buildEvent` FOR A REASON THAT HAS EXPIRED, AND THE
// REASON IS KEPT BECAUSE IT IS THE WHOLE ARGUMENT OF ADR-410. It read:
// "`apps/worker/src/schedule.ts` cites this file at TWO exact coordinates and
// both are above `buildEvent`, so a header insertion of any size breaks a live
// pointer in a file ADR-408's row may not write." **THAT WAS TRUE, AND IT WAS
// THE SMALL END OF THE REAL OBSTRUCTION.** ADR-410 performed the move and found
// that the path `apps/api/src/events.ts` was not merely where this file lived:
// three suites under `apps/worker/test` recovered `EVENT_CATALOGUE` by searching
// that path's TEXT, so no module left behind could satisfy them, and `RI-35`'s
// register named that path as the one module its install probe must skip. Both
// of those pointers, all three suites and that constant were repaired under a
// fence granted for the purpose, and this block is free to sit where it belongs.
//
// THE THIRD ADDRESS WAS PRICED BY ADR-408 AND IS TAKEN BY ADR-410. `apps/api`
// and `apps/worker` share THREE workspace dependencies, `@merit/db`,
// `@merit/ledger` and `@merit/rules-engine`, re-derived from both manifests
// rather than inherited, so an address both arrows reach EXISTED ALREADY rather
// than being a package a founder must first admit through `VG-12`. This package
// is the one ADR-410 ruled, on the writer's shape rather than on convenience:
// `@merit/db` would have made the eventual wiring illegal under
// `apps/worker/test/digests.test.ts` case 10.2, and `@merit/rules-engine`'s
// stated contract is zero I/O while this file's whole purpose is the insert.
// `event-placement.test.ts` binds the intersection, the carrier set and the
// barrel, so a wave that severs the shared dependency or mints a second producer
// lands red rather than quietly making this paragraph false.
//
  ...
  constructor() {
    super(
      'no event writer is installed, so this deployment cannot record an event. The reason is no ' +
        ...
        'module. SO IT IS A FILE ON DISK RATHER THAN A DEPLOYMENT DECISION, and ADR-410 MOVED ' +
        'IT: this producer now lives at `packages/ledger/src/events.ts`, published from that ' +
        'package`s barrel, and `@merit/ledger` is already a dependency of BOTH deployables. THE ' +
        'ADDRESS PART OF THIS BLOCKER IS SPENT AND WHAT IS LEFT IS THE INSTALL ITSELF, which is ' +
        'one file under `apps/worker/src` composing `makeEventSink` over the `SystemTx` that ' +
        'deployable already opens, and which ADR-410 deliberately did not take because it is a ' +
        'slice with its own verification. Refusing is the ' +
 ...
 *
 * IT OPENS NO DOOR, WHICH IS WHY IT CAN LIVE IN A PACKAGE THAT DECLARES NO
 * DEPENDENCY. `apps/api/src/db.ts` is "THE ONE FILE IN THAT DEPLOYABLE THAT
 * NAMES `@merit/db`" (ADR-120) and what that convention protects is the
 * ACQUISITION of a handle: `db.test.ts` asserts that no file but that one takes
 * `firmDb`, `scopedDb`, `systemDb` or `transaction`. This takes none of them,
 * and after ADR-410 it could not: `packages/ledger` cannot name that package at
 * all. **THE PRONOUN WAS `THIS DEPLOYABLE` UNTIL THAT ENTRY** and is replaced
 * rather than left pointing at a directory this file is no longer under. It receives a handle the caller
