...
**Merit gate.** Reinforces **VG-4 + VG-5**: `scopedDb(identity)` is the structural answer, and **any new table ships with its negative-authz test in the same PR or does not merge.** Additional rule: default-deny is the posture, so a table with no explicit access policy is treated as unreachable by the app role, not open.
